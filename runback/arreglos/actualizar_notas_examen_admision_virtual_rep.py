...
try:
    ...
    matriculas = Matricula.objects.filter(status=True, nivel__periodo=periodo, estado_matricula__in=[2,3], inscripcion__carrera__in=[97,98,99,100,101,104,105,96,102,103], inscripcion__inscripcionmalla__malla__inicio__year=2018).distinct().order_by('inscripcion__persona__apellido1', 'inscripcion__persona__apellido2', 'inscripcion__persona__nombres')
    cont = 0
    ...
    for matricula in matriculas:
        ...
        sql_examenes2 = ""
        nota = None
        materiaasignadas = matricula.materiaasignada_set.filter(status=True,materiaasignadaretiro__isnull=True, materia__esintroductoria=False).order_by( 'materia__id').distinct()
        for materiaasignada in materiaasignadas:
            nota=None
            if materiaasignada.materia.asignatura.id == 2750:
                codigoasignatura=2679
            else:
            ...
            sql_examenes2 = """ SELECT DISTINCT test.id AS id_examen, test.name AS nombre_examen,
                                            ...
                                            """ % (str(matricula.inscripcion.persona.identificacion()),
                                                   "%2020-RP-" + str(materiaasignada.materia.asignatura.id) + "%")
