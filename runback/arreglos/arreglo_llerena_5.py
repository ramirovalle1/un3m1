...
def mtfaci(anio):
    ...
    try:
        ...
        for materia in qsmateria:
            ...
            ws.write(row_num, 17, materia.notafinal if not math.isnan(materia.notafinal) else 0, formatoceldacenter)
            if profesor:
                ws.write(row_num, 18, profesor.__str__(), formatoceldacenter)
                ws.write(row_num, 19, profesor.persona.sexo.__str__() if profesor.persona.sexo else '', formatoceldacenter)
            else:
            ...
            if titulos_docente_phd:
                ...
                for titulo in titulos_docente_phd:
                    titulo_phd += f"{titulo},"
                ws.write(row_num, 20, titulo_phd, formatoceldacenter)
            else:
            ...
            if titulos_docente_maestria:
                ...
                ws.write(row_num, 21, titulo_maestria, formatoceldacenter)
            else:
                ws.write(row_num, 21, '', formatoceldacenter)
            row_num += 1
            leido += 1
...
listadoperi = [2016,2017,2018,2019,2020,2021,2022,2023]

# cuenta = 0
# for lper in listadoperi:
#     print(lper)
#     cuenta = cuenta + 1
#     mtfaci(lper)
# mtfaci(2016)
...
if __name__ == "__main__":
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        ...
        # future_1 = executor.submit(mtfaci(2016))
        # future_2 = executor.submit(mtfaci(2017))
        # future_3 = executor.submit(mtfaci(2018))
        # future_4 = executor.submit(mtfaci(2019))
        # future_5 = executor.submit(mtfaci(2020))
        # future_6 = executor.submit(mtfaci(2021))
        # future_7 = executor.submit(mtfaci(2022))
        future_8 = executor.submit(mtfaci(2023))