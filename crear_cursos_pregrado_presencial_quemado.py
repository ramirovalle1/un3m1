...
ids_cursos = [55775,	55771,	55772,	55776,	55773,	55777,	55774,	55770, 55768,	55762,	55764,	55763,	55765,	55767,	55769]
# PRESENCIAL
...
if parent_grupoid > 0:
    ...
    if parent_periodoid > 0:
        ...
        cordinaciones = Coordinacion.objects.filter(pk__in=Materia.objects.values_list('asignaturamalla__malla__carrera__coordinacion__id').filter(nivel__periodo=periodo, idcursomoodle=0, id__in=ids_cursos, status=True).distinct()).distinct()
        for coordinacion in cordinaciones:
            ...
            if parent_coordinacionid > 0:
                ...
                """
                if TIPO_AULA_VIRTUAL == True:
                    carreras = Carrera.objects.filter(pk__in=Materia.objects.values_list('asignaturamalla__malla__carrera').filter(nivel__periodo=periodo, asignaturamalla__malla__carrera__coordinacion=coordinacion,idcursomoodle=0, id__in=ids_cursos, status=True).distinct()).distinct()
                else:
                    carreras = Carrera.objects.filter(pk__in=Materia.objects.values_list('asignaturamalla__malla__carrera').filter(nivel__periodo=periodo, asignaturamalla__malla__carrera__coordinacion=coordinacion,idcursomoodle=0, id__in=ids_cursos, status=True).distinct()).distinct()
                    ...
                    if parent_carreraid > 0:
                        ...
                        niveles = NivelMalla.objects.filter(
                            pk__in=Materia.objects.values_list('asignaturamalla__nivelmalla').filter(
                                nivel__periodo=periodo,idcursomoodle=0, id__in=ids_cursos, status=True).distinct()).distinct()
                        for semestre in niveles:
                            ...
                            if categoryid > 0:
                                ...
                                CREANDO LOS CURSOS
                                """
                                cursos = Materia.objects.filter(nivel__periodo=periodo, id__in=ids_cursos,
                                                                asignaturamalla__nivelmalla=semestre,
                                                                asignaturamalla__malla__carrera=carrera,idcursomoodle=0,
                                                                status=True).distinct().order_by('asignatura__nombre', 'inicio', 'identificacion', 'id')
                                for curso in cursos:
