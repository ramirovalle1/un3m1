...
from helpdesk.forms import HdIncidenciaFrom, HdRequerimientoPiezaPartesForm, HdRequerimientosPiezaPartesForm, \
    SeleccionarActivoForm, DetalleOrdenForm, CerrarOrdenForm, HdMaterial_IncidenteForm
from helpdesk.models import HdSubCategoria, HdDetalle_SubCategoria, HdIncidente, \
    HdDetalle_Grupo, HdDetalle_Incidente, HdEstado, HdEstado_Proceso, HdProceso, HdDirector, HdGrupo,  \
    HdBloqueUbicacion, HdCategoria, HdPiezaPartes, HdRequerimientosPiezaPartes, HdSolicitudesPiezaPartes, HdCausas, \
    HdPrecioSolicitudesPiezaPartes, OrdenTrabajo, DetalleOrdenTrabajo, HdDetalle_Incidente_Ayudantes, HdMaterial_Incidente, HdTipoIncidente
from sga.commonviews import adduserdata, obtener_reporte
from sga.funciones import MiPaginador, log, generar_nombre
from sga.models import Persona
from sagest.models import ActivoFijo,Departamento,HdBloque
    ...
    if request.method == 'POST':
        ...
        if action == 'addmaterialincidente':
            try:
                materialincidente = HdMaterial_Incidente(incidente_id=request.POST['idincidente'],
                                                         ...
                                                         unidadmedida_id=request.POST['idmedida'],
        ...
        if action == 'vermaterialesincidente':
            try:
                ...
                template = get_template("helpdesk_hdagente/consultalistamaterialincidente.html")
        ...
        if action == 'separarincidente':
            try:
                ...
                incidente.save(request)
                agente = HdDetalle_Grupo.objects.filter(status=True, estado=True,persona=int(persona.pk))[0]
                det = HdDetalle_Incidente(incidente=incidente,
                                          agente=agente,
                                          grupo=agente.grupo,
                                          fecharesolucion=datetime.now().date(),
                                          ...
                                          estadoasignacion=1,
                                          responsable_id=HdDetalle_Grupo.objects.get(responsable=True,grupo=agente.grupo).persona.id,
                                          estadoproceso_id=request.POST[
                                              'estadobaja'] if 'estadobaja' in request.POST else None,
                                          estado_id=2)
                det.save(request)
                return JsonResponse({"result": "ok"})
            except Exception as ex:
        ...
        if action == 'detallerequerimiento':
            try:
                ...
                template = get_template("helpdesk_hdagente/detallerequerimiento.html")
                json_content = template.render(data)
        ...
        elif action == 'listadopartes':
            try:
                parte = HdPiezaPartes.objects.values_list('id','hdsolicitudespiezapartes__grupocategoria__descripcion','descripcion').filter(hdsolicitudespiezapartes__isnull=False).distinct()
                if parte.count() > 0:
                    partes = []
                    for r in parte:
                        partes.append({"id": r[0], "valor": r[1] + ' - ' + r[2]})
                    return JsonResponse({"result": "ok", "data": partes})
                return JsonResponse({"result": "bad"})
            except Exception as ex:
                return JsonResponse({"result": "bad"})

        elif action == 'listadopartesdetalle':
            try:
        ...
        if action == 'modaldetalle':
            try:
                if 'id' in request.POST:
                    ...
                    template = get_template("helpdesk_hdagente/modaldetalle.html")
                    json_content = template.render(data)
            ...
            try:
                ...
                template = get_template("helpdesk_hdagente/detalle_activo.html")
                json_content = template.render(data)
                return JsonResponse({"result": "ok", 'html': json_content})
            except Exception as ex:
                return JsonResponse({"result": "bad", "mensaje": u"Error al obtener los datos."})
        ...
        if 'action' in request.GET:
            ...
            if action == 'add':
                try:
                    ...
                    form.add()
                    data['form']=form
                    return render(request, "helpdesk_hdagente/add.html", data)
                except Exception as ex:
                    pass
            ...
            if action == 'resolverincidente':
                try:
                    ...
                    data['form'] = form
                    return render(request, "helpdesk_hdagente/resolverincidente.html", data)
                except Exception as ex:
                    pass
            ...
            if action == 'reasignaragente':
                try:
                    ...
                    form.reasignar()
                    data['form']=form
                    return render(request, "helpdesk_hdagente/reasignaragente.html", data)
                except Exception as ex:
                    pass
            ...
            if action == 'requerimientospiezapartes':
                try:
                    ...
                    data['requerimientospiezapartes'] = incidente.hdrequerimientospiezapartes_set.filter(status=True)
                    return render(request, "helpdesk_hdagente/requerimientospiezapartes.html", data)
                except Exception as ex:
                    pass
            ...
            if action == 'resolverrequerimiento':
                try:
                    ...
                    data['form'] = form
                    return render(request, "helpdesk_hdagente/resolverrequerimiento.html", data)
                except Exception as ex:
                    pass
            ...
            if action == 'addrequerimientopiezaparte':
                try:
                    ...
                    return render(request, "helpdesk_hdagente/addrequerimientopiezaparte.html", data)
                except Exception as ex:
                    pass
            ...
            if action=='delrequerimientopiezaparte':
                try:
                    ...
                    return render(request, "helpdesk_hdagente/delrequerimientopiezaparte.html", data)
                except Exception as ex:
                    pass
            ...
            if action == 'escalarincidente':
                try:
                    ...
                    form.escalar()
                    data['form'] = form
                    return render(request, "helpdesk_hdagente/escalarincidente.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "helpdesk_hdagente/listaordentrabajo.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "helpdesk_hdagente/cerrarorden.html", data)
                except Exception as ex:
                    pass
            ...
            if persona.hddetalle_grupo_set.filter(status=True, estado=True):
                ...
                # mis incidentes
                incidentes=None
                if HdIncidente.objects.filter(status=True).exists():
                    incidentes = HdIncidente.objects.filter(Q(status=True), Q(h_d_incidentes__agente=agente), Q(h_d_incidentes__estadoasignacion=1)).order_by('estado_id', '-fechareporte').distinct()
                    ...
                    data['cargo'] = '(' + persona.distributivopersona_set.get(status=True, estadopuesto=1).denominacionpuesto.descripcion + ')' if persona.distributivopersona_set.filter(status=True, estadopuesto=1) else ''
                    data['cant_mis_incidentes'] =hd= HdIncidente.objects.filter(Q(h_d_incidentes__agente=agente), Q(status=True), (Q(estado=1)|Q(estado=2))).exclude(Q(h_d_incidentes__estadoasignacion=3) | Q(h_d_incidentes__estadoasignacion=4)).count()
            ...
            data['estados'] = HdEstado.objects.all()
            return render(request, "helpdesk_hdagente/view.html", data)
