...
try:
    ...
    # for materia in Materia.objects.filter(status=True, nivel__periodo=periodo, esintroductoria=False, asignaturamalla__malla__carrera__in=[47, 51, 52, 55, 64, 65, 66, 67, 68, 73, 74, 82, 83, 84, 85, 86, 87, 107, 108, 124] ):
    for materia in Materia.objects.filter(id=24079  ,status=True, nivel__periodo=periodo, esintroductoria=False):
        for alumno in materia.materiaasignada_set.filter(matricula__id=208260,matricula__estado_matricula__in=[2,3], retiramateria=False).order_by('matricula__inscripcion__persona__apellido1', 'matricula__inscripcion__persona__apellido2','matricula__inscripcion__persona__nombres'):
