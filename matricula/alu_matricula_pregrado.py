        ...
        if 'action' in request.POST:
            ...
            if action == 'loadInitialData':
                ...
                try:
                    ...
                    template = get_template("matricula/pregrado/view_valores_pendientes.html")
                ...
                try:
                    ...
                    if not inscripcion.matricula_set.values("id").filter(automatriculapregrado=True, termino=False, nivel__periodo=periodo).exists():
                        return JsonResponse({"result": "bad", "mensaje": u"No se encontro matricula pendiente de aceptar."})
                    matricula = inscripcion.matricula_set.filter(automatriculapregrado=True, termino=False, nivel__periodo=periodo)[0]
        ...
        if 'action' in request.GET:
            ...
            try:
                ...
                if periodomatricula.periodo and inscripcion.tiene_automatriculapregrado_por_confirmar(periodomatricula.periodo):
                    ...
                    return render(request, "matricula/pregrado/confirmar_automatricula.html", data)
                ...
                if periodomatricula.valida_coordinacion:
                    if not inscripcion.coordinacion.id in periodomatricula.coordinaciones().values_list("id", flat=True):
                ...
                if hoy <= nivel.fechatopematriculaes:
                    if hoy > nivel.fechatopematriculaex:
                        if periodomatricula.valida_proceos_matricula_especial:
                            raise NameError(f"Estimado/a estudiante, el proceso de matrícula especial termina el {nivel.fechatopematriculaes.__str__()}. <br>Verificar en el módulo <a href='/alu_solicitudmatricula' class='bloqueo_pantalla'>Solicitud de Matrícula</a>")
                        else:
                            raise NameError(f"Estimado/a estudiante, el proceso de matrícula especial termina el {nivel.fechatopematriculaes.__str__()}.")
                else:
                    if hoy > nivel.fechatopematriculaes:
                ...
                data['num_va_ultima_matricula'] = inscripcion.num_va_ultima_matricula(periodomatricula.num_matriculas)
