...
for i in range(200):
    ...
    facturas =Factura.objects.filter(total__gt=0).filter(Q(autorizada=False) | Q(enviadacliente=False), valida=True).distinct().values_list('id', flat=True).order_by('-id')
