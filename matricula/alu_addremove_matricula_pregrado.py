...
from matricula.funciones import valid_intro_module_estudiante, get_nivel_matriculacion, \
    ...
    generateCodeDeleteMatricula, reenvioCodeDeleteMatricula, get_client_ip
    ...
    if request.method == 'POST':
        ...
        if 'action' in request.GET:
            ...
            try:
                ...
                periodomatricula = None
                matricula = None
                if not PeriodoMatricula.objects.values_list('id').filter(status=True, activo=True, periodo=periodo, tipo=2).exists():
                    raise NameError(u"Estimado/a estudiante, el periodo de matriculación se encuentra inactivo")
                periodomatricula = PeriodoMatricula.objects.filter(status=True, activo=True, periodo=periodo, tipo=2)
                ...
                return render(request, "matricula_addremove/pregrado/view.html", data)
            except Exception as ex:
