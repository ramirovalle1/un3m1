...
from sagest.forms import HistorialJornadaTrabajadorForm, JornadaLaboralForm, CambiarMarcadaForm, JustificarMarcadaForm
...
def view(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'importar_logindividual':
            ...
            try:
                ...
                if 'archivo' in request.FILES:
                    ...
                    if d.size > 4194304:
                        ...
                        if not (ext == '.pdf' or ext == '.png' or ext == '.docx' or ext == '.doc'  or ext == '.jpg' or ext == '.jpeg'):
                            return JsonResponse(
                                {"result": "bad", "mensaje": u"Error, solo archivos .pdf, doc, png, jpg, jpeg."})
                ...
                if f.is_valid():
                    ...
                    if 'archivo' in request.FILES:
                        ...
                        newfile._name = generar_nombre("historialjornada_", newfile._name)
                        historial.archivo_autorizacion = newfile
                        historial.save(request)
            ...
            try:
                ...
                if f.is_valid():
                    ...
                    if 'archivo' in request.FILES:
                        newfile = request.FILES['archivo']
                        newfile._name = generar_nombre("historialjornada_", newfile._name)
                        jornada.archivo_autorizacion = newfile
                        jornada.save(request)
            ...
            try:
                fechai = convertir_fecha_invertida(request.POST['fechai'])
                fechaf = convertir_fecha_invertida(request.POST['fechaf'])
            ...
            try:
                ...
                if DistributivoPersona.objects.filter(persona=personad, status=True).exists():
                    data['distributivo'] = DistributivoPersona.objects.filter(persona=personad, status=True)[0]
                anio = request.POST['anio']
                ...
                template = get_template("th_marcadas/detallejornadatrab.html")
        ...
        #                                 marcadadia.save(request)
        #                             if not marcadadia.registromarcada_set.filter(entrada=entrada).exists():
        #                                 registro_m = RegistroMarcada(marcada=marcadadia,
        #                                                            entrada=entrada,
        #                                                            salida=salida,
        #                                                            segundos=(salida - entrada).seconds)
        #                                 registro_m.save(request)
        #                             marcadadia.actualizar_marcadas()
        #                         else:
        #                             entrada = dl.time
        #                             marini += 1
        ...
        # elif action == 'addmarcadajustificar':
        #     try:
        #         if puede_realizar_accion_is_superuser(request, 'sagest.puede_agregar_marcada_log'):
        #             fecha =  request.POST['dia'] if 'dia' in request.POST and request.POST['dia'] else None
        #             jornada =  request.POST['jorn'] if 'jorn' in request.POST and request.POST['jorn'] else None
        #             per =  request.POST['per'] if 'per' in request.POST and request.POST['per'] else None
        #
        #             if not fecha:
        #                 return JsonResponse({"result": "bad", "mensaje": u"Error al guardar los datos."})
        #             if not jornada:
        ...
        #                 return JsonResponse({"result": "bad", "mensaje": u"Error al guardar los datos."})
        #
        #             trabajador = Persona.objects.get(pk=per)
        #             fecha = datetime.strptime(request.POST['dia'], '%Y-%m-%d').date()
        #             numero_dia = fecha.weekday() + 1
        #             jornada = Jornada.objects.get(pk=int(jornada))
        ...
        #
        #
        #                 if trabajador.logdia_set.filter(fecha=fecha).exists():
        #                     logdia = trabajador.logdia_set.filter(fecha=fecha).first()
        #                     if logdia.persona.historialjornadatrabajador_set.filter(fechainicio__lte=logdia.fecha, fechafin__gte=logdia.fecha).exists():
        #                         logdia.jornada = logdia.persona.historialjornadatrabajador_set.filter(fechainicio__lte=logdia.fecha, fechafin__gte=logdia.fecha).order_by('fechainicio')[0].jornada
        #                     elif logdia.persona.historialjornadatrabajador_set.filter(fechainicio__lte=logdia.fecha, fechafin=None).exists():
        #                         logdia.jornada = logdia.persona.historialjornadatrabajador_set.filter(fechainicio__lte=logdia.fecha, fechafin=None)[0].jornada
        ...
        #                     logdia.procesado = False
        #                 else:
        #                     if not LogDia.objects.filter(persona=trabajador,fecha=fecha).exists():
        #                         logdia = LogDia(persona=trabajador,
        #                                         fecha=fecha,
        #                                         cantidadmarcadas=1)
        #                         logdia.save(request)
        #                     else:
        #                         logdia = LogDia.objects.get(persona=trabajador,fecha=fecha)
        #
        #                 if not logdia.logmarcada_set.filter(time=time).exists():
        #                     registro = LogMarcada(logdia=logdia,
        #                                           time=time,
        ...
        #                     registro = logdia.logmarcada_set.filter(time=time).order_by('time').first()
        #
        #                 for l in LogDia.objects.filter(persona=trabajador, status=True, procesado=False).order_by("fecha"):
        #                     if not l.jornada:
        #                         if l.persona.historialjornadatrabajador_set.filter(fechainicio__lte=l.fecha,
        #                                                                            fechafin__gte=l.fecha).exists():
        #                             l.jornada = l.persona.historialjornadatrabajador_set.filter(fechainicio__lte=l.fecha,
        ...
        elif action == 'justificarmarca':
            try:
                form = JustificarMarcadaForm(request.POST)
                    ...
                    if puede_realizar_accion_is_superuser(request, 'sagest.puede_agregar_marcada_log'):
                        ...
                        for det in detalle_jornada:
                            ...
                            if trabajador.logdia_set.filter(fecha=fecha).exists():
                                ...
                                if not LogDia.objects.filter(persona=trabajador, fecha=fecha).exists():
                                    logdia = LogDia(persona=trabajador,
                                                    fecha=fecha, observacion = observacion, diajustificado = True,
                                                    cantidadmarcadas=1)
                                    ...
                                    if 'archivo' in request.FILES:
                                        newfile = request.FILES['archivo']
                                        newfile._name = generar_nombre("justificacion_", newfile._name)
                                        logdia.archivo = newfile
                                        logdia.save(request)
                                else:
                                    logdia = LogDia.objects.get(persona=trabajador, fecha=fecha)
        ...
        if 'action' in request.GET:
            ...
            if action == 'importar_vc':
                try:
                    data['title'] = u'Importar datos vinculación'
                    data['form'] = ImportarArchivoXLSForm()
                    return render(request, "th_marcadas/importar_vc.html", data)
                except Exception as ex:
                    pass
            ...
            if action == 'jornadastrabajador':
                try:
                    ...
                    return render(request, "th_marcadas/jornadastrabajador.html", data)
                except Exception as ex:
                    pass
            ...
            if action == 'detallejornadatrabajador':
                try:
                    ...
                    data['jornadas'] = persona.historialjornadatrabajador_set.all()
                    return render(request, "th_marcadas/detallejornadastrabajador.html", data)
                except Exception as ex:
                    pass
            ...
            if action == 'logmarcadas':
                try:
                    ...
                    if Jornada.objects.filter(nombre=jornada).exists():
                        data['idjornada'] = int(Jornada.objects.get(nombre=jornada).id)
                    else:
                        data['idjornada'] = None
                    if persona.distributivopersona_set.filter(status=True ).exists():
                        data['distributivo'] = distributivo = persona.distributivopersona_set.filter(status=True )[0]
                    data['anios'] = persona.lista_anios_trabajados_log()
            ...
            if action == 'add':
                try:
                    ...
                    data['persona'] = persona = Persona.objects.get(pk=int(request.GET['id']))
                    if persona.historialjornadatrabajador_set.all().exists():
                        data['form'] = HistorialJornadaTrabajadorForm()
                    else:
                        form = HistorialJornadaTrabajadorForm()
                        form.ocultarcampos_archivo()
                        data['form'] = form

                    return render(request, "th_marcadas/add.html", data)
                except Exception as ex:
                    pass
            ...
            if action == 'editjornadatrab':
                try:
                    ...
                    data['jornada'] = jornada = HistorialJornadaTrabajador.objects.get(pk=int(request.GET['id']))
                    # data['distributivo'] = DistributivoPersona.objects.get(pk=int(request.GET['per']))
                    data['persona'] = persona = Persona.objects.get(pk=int(request.GET['per']))
                    form = HistorialJornadaTrabajadorForm(initial={'jornada': jornada.jornada,
                                                                   'fechainicio': jornada.fechainicio,
                                                                   'fechafin': jornada.fechafin if jornada.fechafin else datetime.now().date()})
                    if HistorialJornadaTrabajador.objects.filter(persona = persona, status = True).count() == 1:
                        form.ocultarcampos_archivo()

                    form.editar()
                    data['form'] = form
                    return render(request, "th_marcadas/editjornadatrab.html", data)
                except Exception as ex:
                    pass
            ...
            elif action == 'excelmarcadas':
                try:
                    __author__ = 'Unemi'

                    output = io.BytesIO()
                    workbook = xlsxwriter.Workbook(output)
                    ws = workbook.add_worksheet('marcadas')
                    ...
                    id_dis = DistributivoPersona.objects.values_list('persona_id').filter(status=True)
                    logdia = LogDia.objects.filter(fecha__gte=fechainicio,fecha__lte=fechafinal,status=True,persona_id__in=id_dis).order_by('persona_id')

                    ws.write(0, 0, 'NOMBRE', formatoceldagris)
                    ws.write(0, 1, 'CEDULA', formatoceldagris)
                    ws.write(0, 2, 'CARGO', formatoceldagris)
                    ws.write(0, 3, 'DEPARTAMENTO', formatoceldagris)
                    ws.write(0, 4, 'JORNADA', formatoceldagris)
                    ws.write(0, 5, 'FECHA', formatoceldagris)
                    ws.write(0, 6, 'MARCADA1', formatoceldagris)
                    ws.write(0, 7, 'MARCADA2', formatoceldagris)
                    ws.write(0, 8, 'MARCADA3', formatoceldagris)
                    ws.write(0, 9, 'MARCADA4', formatoceldagris)
                    ws.write(0, 10, 'MARCADA5', formatoceldagris)
                    ws.write(0, 11, 'MARCADA6', formatoceldagris)
                    ws.write(0, 12, 'MARCADA7', formatoceldagris)
                    ws.write(0, 13, 'MARCADA8', formatoceldagris)
                    ws.write(0, 14, 'MARCADA9', formatoceldagris)
                    ws.write(0, 15, 'MARCADA10', formatoceldagris)
                    ws.write(0, 16, 'MARCADA11', formatoceldagris)
                    ws.write(0, 17, 'MARCADA12', formatoceldagris)
                    cont = 1
                    for dia in logdia:
                        ws.write(cont, 0, str(dia.persona))
                        ws.write(cont, 1, str(dia.persona.identificacion()))
                        ws.write(cont, 2, str(dia.persona.mi_cargo_actual().denominacionpuesto))
                        ws.write(cont, 3, str(dia.persona.mi_cargo_actual().unidadorganica))
                        ws.write(cont, 4, str(dia.jornada))
                        ws.write(cont, 5, str(dia.fecha))
                        marcadas = dia.logmarcada_set.filter(status=True).order_by('time')
                        cont2=6
                        for mar in marcadas:
                            if mar.time:
                                ws.write(cont, cont2, mar.time.time().strftime("%H:%M:%S"))
                                cont2 += 1
                        cont += 1
                    workbook.close()
                    output.seek(0)
                    filename = 'reporte_marcadas.xlsx'
            ...
            if action == 'eliminarjornadatrab':
            ...
            elif action == 'detallejustificación':
                try:
                    logdia = LogDia.objects.get(pk=request.GET['id'])
                    archivo = ''
                    if logdia.archivo:
                        archivo = logdia.archivo.url
                    form = JustificarMarcadaForm(initial={'fecha':logdia.fecha, 'observacion': logdia.observacion})
                    form.bloqueofecha_observación()
                    form.ocultarcampos_archivo()
                    data['form'] = form
                    data['url'] = archivo
                    template = get_template("th_marcadas/modal/detallejustificacion.html")
                    return JsonResponse({"result": True, 'data': template.render(data)})
                except Exception as ex:
                    pass
                ...
                try:
                    form = JustificarMarcadaForm()
                    data['id'] = request.GET['id']
                    data['idjor'] = request.GET['idjor']
                    data['form'] = form
                    template = get_template("th_marcadas/modal/justificarfalta.html")
                    return JsonResponse({"result": True, 'data': template.render(data)})
                except Exception as ex:
                    pass
...
def extraervalores(id, mes, anio, h):
    ...
    distributivo=None
    if DistributivoPersona.objects.values('id').filter(persona=personad, status=True).exists():
        distributivo = DistributivoPersona.objects.filter(persona=personad, status=True)[0]
