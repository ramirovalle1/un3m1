...
def view(request):
    ...
    if request.method == 'POST':
        ...
        if 'action' in request.GET:
            ...
            if action == 'criteriosevaluacion':
                try:
                    data['title'] = u'Criterios de Evaluación de Investigación'
                    data['cronograma'] = cronograma = CronogramaEvaluacionInvestigacion.objects.get(pk=int(encrypt(request.GET['id'])))
                    data['criterioscronograma'] = cronograma.criterios_evaluacion()
                    fechainicioperiodo = cronograma.periodos_evaluacion()[0].periodo.inicio

                    articulospublicados = ArticuloInvestigacion.objects.select_related().\
                                           annotate(duracion_meses=ExpressionWrapper(fechainicioperiodo - F('fechapublicacion'), output_field=DurationField())).\
                                           filter((Q(
                                                    ...
                                                    participantesarticulos__status=True,
                                                    duracion_meses__lte=timedelta(days=365)).order_by('-fechapublicacion')

                    data['articulospublicados'] = articulospublicados

                    data['articulosaceptados'] = [1, 2]
                    data['borradoresarticulos'] = [1, 2]
                    data['ponencias'] = [1]
                    data['proyectosinvestigacion'] = [1, 2]
                    data['direccionestesis'] = [1, 2]
                    data['revisionesarticulos'] = [1, 2]
                    data['redesgruposinvexternos'] = [1, 2]
                    ...
                    return render(request, "pro_evaluacioninvestigacion/criterioevaluacion.html", data)
                except Exception as ex:
                    pass
            ...
            try:
                ...
                return render(request, "pro_evaluacioninvestigacion/view.html", data)
            except Exception as ex:
                pass
