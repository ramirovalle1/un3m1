...
parent_grupoid = 0
tipourl = 1
periodo = Periodo.objects.get(pk=86)

bgrupo = moodle.BuscarCategoriasid(periodo, tipourl, 4)
...
if parent_grupoid > 0:
    ...
    if parent_periodoid > 0:
        ...
        """
        cordinaciones = Coordinacion.objects.filter(carrera__in=Materia.objects.values_list('asignaturamalla__malla__carrera').filter(nivel__periodo=periodo, status=True).distinct()).distinct()
        ...
        for coordinacion in cordinaciones:
            ...
            if parent_coordinacionid > 0:
                ...
                """
                carreras = Carrera.objects.filter(pk__in=Materia.objects.values_list('asignaturamalla__malla__carrera').filter(nivel__periodo=periodo, asignaturamalla__malla__carrera__coordinacion=coordinacion, status=True).distinct()).distinct().exclude(modalidad=3)
                ...
                for carrera in carreras:
                    ...
                    if parent_carreraid > 0:
                        ...
                        """
                        niveles = NivelMalla.objects.filter(pk__in=Materia.objects.values_list('asignaturamalla__nivelmalla').filter(id__in=[31779],nivel__periodo=periodo, asignaturamalla__malla__carrera=carrera, status=True).distinct()).distinct()
                        for semestre in niveles:
                            ...
                            if categoryid > 0:
                                ...
                                CREANDO LOS CURSOS
                                """
                                cursos = Materia.objects.filter(id__in=[31779] ,nivel__periodo=periodo,status=True,asignaturamalla__nivelmalla=semestre, asignaturamalla__malla__carrera=carrera).order_by('asignatura__nombre', 'inicio', 'identificacion', 'id').exclude(nivel__sesion__id=13)
                                for curso in cursos:
                                    ...
                                    if bcurso['courses']:
                                        ...
                                        bcurso = moodle.CrearCursos(periodo, tipourl, u'%s' % curso.nombre_completo(), u'%s,[%s] - %s[%s]' % (curso.asignatura.nombre,curso.identificacion, curso.paralelo, curso.id), categoryid , idnumber_curso, summary, startdate, enddate, numsections)
                                        cursoid = bcurso[0]['id']
                                    print('********Curso: %s' % curso)
                                    if cursoid > 0:
                                        ...
                                        if AGREGAR_SILABO:
                                            curso.crear_actualizar_silabo_curso_posgrado()
