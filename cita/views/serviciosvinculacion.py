...
def view(request):
    ...
    if not 'sistema' in request.GET:
        sistema_tipo = data['tiposistema']
    else:
        sistema_tipo = request.GET['sistema']
    if not DepartamentoServicio.objects.values('id').filter(tiposistema=sistema_tipo, status=True).exists():
        return redirect('/')
    data['gruposervicio'] = gruposervicio = DepartamentoServicio.objects.get(tiposistema=sistema_tipo)
    # AQUI ACTUALIZAMOS LAS VARIABLES DE SESION PARA QUE SE ACTUALICE SEGUN EL TIPO DE SISTEMA A ACCEDER, SIN DESLOGEAR
    if request.session['tiposistema'] != gruposervicio.tiposistema:
        request.session['tiposistema'] = gruposervicio.tiposistema
        request.session['tipoentrada'] = gruposervicio.tiposistema
        request.session['url_entrada'] = gruposervicio.url_entrada
        data['tiposistema'] = request.session['tiposistema']

    data['url_entrada'] = gruposervicio.url_entrada
    if request.method == 'POST':
        ...
        # ESTUDIOS
            ...
            with transaction.atomic():
                try:
                    campos_requeridos = ['fecha', 'idservicio', 'familiar', 'responsable', 'idmotivo', 'horario']
                    for campo in campos_requeridos:
                        if campo not in request.POST or not request.POST[campo].strip():
                            return JsonResponse({'result': False, 'msg': f"Por favor, complete los campos faltantes antes de continuar."})

                    if request.POST["idmotivo"] == "79" and not request.POST['descripcionmotivo'].strip():
                        return JsonResponse({'result': False, 'msg': f"Descripcion de motivo obligatorio."})
                    ...
                    motivo = MotivoCita.objects.get(id=idmotivo)
                    lista = json.loads(request.POST['listahorarios'])
                    if not request.POST['horario']:
                        transaction.set_rollback(True)
                        return JsonResponse({"result": False, "msg": "Seleccione un horario a reservar."})
                    ...
                    if not turnosdisponible(horario, fecha):
                        return JsonResponse({"result": False, 'funcion': True, 'msg': 'Turno ocupado'})
                    ...
                    cita = PersonaCitaAgendada(persona=persona,
                                               ...
                                               motivoconsulta_id = idmotivo,
                                               descripcionmotivo=request.POST['descripcionmotivo'])
                    cita.save(request)
                    siteurl = encrypt(horario.servicio.serviciocita.departamentoservicio.id)
                    #serviciourl = encrypt(idservicio)
                    serviciourl = encrypt(horario.servicio.serviciocita.id)
                    tourl = f"{request.path}?site={siteurl}&servicio={serviciourl}&action=agendarcita"
                    return JsonResponse({"result": True, "to": tourl})
                except Exception as ex:
                    transaction.set_rollback(True)
                    return JsonResponse({'result': False, 'msg': f'Error al guardar los datos {ex}'})
        ...
        if 'action' in request.GET:
            ...
            if action == 'serviciosvinculacion':
                try:
                    template, data = serviciosvinculacion(data, request)
            ...
            elif action == 'acercanosotros':
                try:
                    template, data = acercanosotros(data, request)
                    return render(request, template, data)
                except Exception as ex:
                    messages.error(request, str(ex))
            elif action == 'eventos':
            ...
            # FORMACIÓN ACADEMICA
                ...
                try:
                    ...
                    data['persona'] = persona
                    data['servicioid'] = int(encrypt(request.GET['servicio']))
                    template, data = agendarcita(data, request)
                    return render(request, template, data)
                except Exception as ex:
                    pass
            ...
            if action == 'cargarcalendario':
                try:
                    ...
                    lista = {}
                    listahorarios = []
                    servicios = ServicioConfigurado.objects.filter(
                        serviciocita__id=id,
                        serviciocita__status=True,
                        serviciocita__departamentoservicio__status=True,
                        mostrar=True,
                        soloadministrativo=False
                    )
                    if not servicios.exists():
                        return JsonResponse({"result": 'bad', 'mensaje': 'No dispone de citas.'})

                    data['servicio'] = servicio = servicios.first()
                    #data['servicio'] = servicio = ServicioConfigurado.objects.get(serviciocita__id=id, serviciocita__status=True, serviciocita__departamentoservicio__status=True, mostrar=True, soloadministrativo=False)
                    # finicio = hoy - timedelta(days=servicio.numdias)
                    data['horarios'] = horarios = HorarioServicioCita.objects.filter(status=True,
                                                                                     responsableservicio__servicio__serviciocita_id=id,
                                                                                     mostrar=True,
                                                                                     fechafin__gte=hoy,
                        ...
                        if comienzo and fin is False:
                            ...
                            if fecha >= hoy:
                                ...
                                diccionario = {'dia': s_dia, 'turnos': turnos, 'listahorario': listhorario,
                                               'sinhorario': sinhorario, 'fecha': fecha,
            ...
            if action == 'addcita':
                try:
                    id = int(request.GET['idservicio'])
                    servicio = ServicioConfigurado.objects.get(serviciocita__id=id, mostrar = True)
                    ...
                    template = get_template("serviciosvinculacion/formagendarcita.html")
                    return JsonResponse({"result": True, 'data': template.render(data)})
                except Exception as ex:
            ...
            #     noticiaid = request.GET['noticiaid']
            #     noticia = NoticiasVinculacion.objects.get(id=noticiaid)
            #     file_path = noticia.archivo.path
            ...
            #             response['Content-Disposition'] = f'attachment; filename="{noticia.archivo.name}"'
            #             return response
