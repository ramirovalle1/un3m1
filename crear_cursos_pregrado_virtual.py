...
if parent_grupoid > 0:
    ...
    if parent_periodoid > 0:
        ...
        cordinaciones = Coordinacion.objects.filter(pk__in=Materia.objects.values_list('asignaturamalla__malla__carrera__coordinacion__id').filter(nivel__periodo=periodo, status=True,asignaturamalla__malla__carrera__coordinacion__id__lte=5).distinct()).distinct()
        for coordinacion in cordinaciones:
            ...
            if parent_coordinacionid > 0:
                ...
                carreras = Carrera.objects.filter(pk__in=Materia.objects.values_list('asignaturamalla__malla__carrera').filter(nivel__periodo=periodo, asignaturamalla__malla__carrera__coordinacion=coordinacion, status=True).exclude(asignaturamalla__malla_id__in=eMallasIngles).distinct()).distinct()
                for carrera in carreras:
                    ...
                    if parent_carreraid > 0:
                        ...
                        niveles = NivelMalla.objects.filter(pk__in=Materia.objects.values_list('asignaturamalla__nivelmalla').filter(nivel__periodo=periodo, status=True,asignaturamalla__malla__carrera__coordinacion=coordinacion).distinct()).distinct()
                        for semestre in niveles:
                            try:
                                ...
                                if categoryid > 0:
                                    ...
                                    cursos = Materia.objects.filter(nivel__periodo=periodo, asignaturamalla__nivelmalla=semestre, asignaturamalla__malla__carrera=carrera,status=True,asignaturamalla__malla__carrera__coordinacion=coordinacion).exclude(asignaturamalla__malla_id__in=eMallasIngles).distinct().order_by('asignatura__nombre', 'inicio', 'identificacion', 'id')
