...
def coordinacion2():
    for carrera in Materia.objects.filter(status=True, nivel__periodo_id=177, cerrado=False,
                                          asignaturamalla__malla__carrera__coordinacion=2).exclude(
        modeloevaluativo_id=27).values_list('asignaturamalla__malla__carrera_id', flat=True).distinct():
        importar_cerrar_materias(carrera)

#coordinacion2()

def actualizar_nivel_inscripcion_malla5():
    matriculas = Matricula.objects.filter(status=True, nivel__periodo_id=177, inscripcion__carrera_id__in=[156, 130] )
...
actualizar_nivel_inscripcion_malla5()
