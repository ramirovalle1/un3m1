    ...
    if request.method == 'POST':
        ...
        if action == 'addaprobacion':
            try:
                ...
                cabecera.estado=variable_valor('PENDIENTE_GRUPO_INVESTIGACION') if int(request.POST['estado'])==variable_valor('APROBADO_GRUPO_INVESTIGACION') else variable_valor('RECHAZADO_GRUPO_INVESTIGACION')
                ...
                log(u'Aprobar solicitud de grupo investigacion(Director de Carrera): %s - [%s]' % (detalle,detalle.id), request, "add")
        ...
        if 'action' in request.GET:
            ...
            if action == 'detallegrupo':
                try:
                    data['title'] = 'Detalles del grupo de investigación'
                    ...
                    data['url_regresar'] = 'adm_solaprobar_dr'
            ...
            data['title'] = u'Aprobar solicitud de grupo investigación(Director).'
            search = None
            ids = None
            if Departamento.objects.filter(responsable=persona, permisogeneral=True, integrantes__isnull=False,status=True).exists():
            ...
            if 's' in request.GET:
                ...
                if len(ss) == 1:
                    grupos = SolicitudGrupoInvestigacion.objects.filter((Q(codirector__persona__nombres__icontains=search) |
                                                                         ...
                                                                         Q(director__persona__distributivopersona__unidadorganica__in=departamento)&
                                                                        (Q(estado=variable_valor('SOLICITADO_GRUPO_INVESTIGACION')) | Q(estado=variable_valor('APROBADO_GRUPO_INVESTIGACION')) |
                                                                         Q(estado=variable_valor('PENDIENTE_GRUPO_INVESTIGACION'))|
                                                                         Q(estado=variable_valor('RECHAZADO_GRUPO_INVESTIGACION')))).distinct(). \
                    ...
                    grupos = SolicitudGrupoInvestigacion.objects.filter(Q(nombre__icontains=ss[0]) & Q(nombre__icontains=ss[1]) & Q(status=True) &
                                                                       ...
                                                                       (Q(estado=variable_valor('SOLICITADO_GRUPO_INVESTIGACION')) | Q(estado=variable_valor('APROBADO_GRUPO_INVESTIGACION'))|
                                                                        Q(estado=variable_valor('PENDIENTE_GRUPO_INVESTIGACION')) |
                                                                        Q(estado=variable_valor('RECHAZADO_GRUPO_INVESTIGACION')))).distinct().order_by('nombre')
            else:
                grupos = SolicitudGrupoInvestigacion.objects.filter(Q(status=True),Q(director__persona__distributivopersona__unidadorganica__in=departamento),
                                                                   (Q(estado=variable_valor('SOLICITADO_GRUPO_INVESTIGACION'))|Q(estado=variable_valor('APROBADO_GRUPO_INVESTIGACION'))|
                                                                    Q(estado=variable_valor('PENDIENTE_GRUPO_INVESTIGACION')) | Q(estado=variable_valor('RECHAZADO_GRUPO_INVESTIGACION')))).distinct().order_by('estado','-fecha_creacion')
            ...
            data['pendiente'] = variable_valor('PENDIENTE_GRUPO_INVESTIGACION')
            data['aprobado'] = variable_valor('APROBADO_GRUPO_INVESTIGACION')
            return render(request, 'adm_solicitudaprobacioninvestigacion/aprobar_director.html', data)
