...
from sga.funciones import log, generar_nombre, MiPaginador, convertir_fecha, convertir_hora, variable_valor, null_to_decimal, \
...
from .formmodel import ReAgendarAgendaPracticasTutoriasForm, FinalizarAgendaTutoriaForm, AgendaPracticasTutoriaForm
...
CRITERIO_PROYECTO_INVESTIGACION_CONVENIO = 25
...
def view(request):
    ...
    if request.method == 'POST':
        if 'action' in request.POST:
            ...
            if action == 'adddetallebitacoramultiple':
                try:
                    bitacora = BitacoraActividadDocente.objects.get(id=request.POST['id'])
                    fechainicio, fechafin = convertir_fecha_invertida(request.POST['fini']), convertir_fecha_invertida(request.POST['ffin'])
                    horainicio, horafin = convertir_hora(request.POST['hini']), convertir_hora(request.POST['hfin'])
                    mensajeerror, errorcounter, counter = '<p>', 0, 0
                    newfile = None
                    if fechainicio.month != bitacora.fechafin.month or fechafin.month != bitacora.fechafin.month:
                        raise NameError(f"El mes seleccionado es incorrecto")

                    if 'archivo' in request.FILES:
                        if newfile := request.FILES['archivo']:
                            newfile._name = generar_nombre("detallebitacora_", newfile._name)
                            if newfile.size > 12582912:
                                raise NameError(f"El archivo no puede ser mayor a 12 MB")

                    diashabilitados = [1, 2, 3, 4, 5]
                    diashorarioactividades = bitacora.criterio.claseactividad_set.filter(actividaddetallehorario__vigente=True, status=True).values_list('dia', flat=True).order_by('dia').distinct()
                    if 6 in diashorarioactividades:
                        diashabilitados += [6]
                    listadodiasregistro = list(daterange(fechainicio, fechafin + timedelta(1)))
                    if len(listadodiasregistro) > 1:
                        diashabilitados = diashorarioactividades

                    for fecha in listadodiasregistro:
                        try:
                            fechaformato = fecha.strftime("%d/%m/%Y")
                            diasemana = fecha.weekday() + 1
                            if diasemana in diashabilitados:
                                if not periodo.diasnolaborable_set.values('id').filter(fecha=fecha, status=True).exists():
                                    if claseactividad := ClaseActividad.objects.filter(dia=fecha.weekday() + 1, turno__comienza__lt=horafin, turno__termina__gt=horainicio, actividaddetallehorario__vigente=True, detalledistributivo__distributivo=bitacora.criterio.distributivo, status=True).exclude(detalledistributivo=bitacora.criterio).first():
                                        errorcounter += 1
                                        criterios = [claseactividad.detalledistributivo.criteriodocenciaperiodo, claseactividad.detalledistributivo.criteriovinculacionperiodo, claseactividad.detalledistributivo.criterioinvestigacionperiodo, claseactividad.detalledistributivo.criteriogestionperiodo]
                                        criteriochoquehorario = str(next((x.criterio for x in criterios if x), None)).lower()
                                        raise NameError(f"{fechaformato} — \
                                                        choque de horario <i class='fa fa-info text-info' title='{criteriochoquehorario}'></i>")

                                    detalle = DetalleBitacoraDocente(bitacoradocente=bitacora,
                                                                     fecha=fecha,
                                    ...
                                    if counter == 1 and newfile:
                                        newfile = "%s" % detalle.archivo
                                else:
                                    errorcounter += 1
                                    raise NameError(f"{fechaformato} — feriado")
                            else:
                                errorcounter += 1
                                raise NameError(f"{fechaformato} — {'fín de semana' if diasemana in (6, 7) else 'fuera de horario de actividades'}")
                        except Exception as ex:
                            mensajeerror += f'<br>{ex}'
                    mensajeerror += '</p>'
                    return JsonResponse({'result': 'ok', 'errorcounter': errorcounter, 'mensajeerror': mensajeerror if errorcounter else ''})
                except Exception as ex:
                    return JsonResponse({'result': 'bad', 'mensaje': f'{ex}'})

            if action == 'deletedetallebitacoramultiple':
                try:
                    bitacora = BitacoraActividadDocente.objects.get(id=request.POST['id'])
                    for criterio in DetalleBitacoraDocente.objects.filter(id__in=json.loads(request.POST['objs']), bitacoradocente=bitacora, status=True):
                        criterio.delete()
            ...
            if action == 'tutoriaadicional':
                try:
                    ...
                    template = get_template("pro_cronograma/addtutoriaadicional.html")
            ...
            if action == 'detallecongreso':
                try:
                    ...
                    template = get_template("pro_cronograma/detallecongreso.html")
            ...
            if action == 'generanumeroconvenio':
                try:
                    ...
                    if not capacitacion.numeroconvenio:
                        ...
                        capacitacion.firmadocente = False
                        capacitacion.save(request)
                        log(u'Genero una nueva solicitud de capacitacion/actualización: %s' % capacitacion, request, "edit")
            ...
            if action == 'conveniodevengacion_pdf_old':
                try:
                    ...
                    return conviert_html_to_pdf(
                        'pro_cronograma/conveniodevengacion_pdf_new.html',
                        {
                            'pagesize': 'A4',
                            'data': data,
                        }
                    ...
                    return HttpResponseRedirect("/pro_cronograma?info=%s" % "Error al generar el reporte")
            ...
            if action == 'conveniodevengacion_pdf':
                ...
                try:
                    ...
                    data['archivo_url'] = capacitacion.archivoconvenio.url
            ...
            if action == 'firmaconvenio':
                ...
                idinforme = int(encrypt(request.POST['id']))
                ...
                try:
                    ...
                    url_archivo = (SITE_STORAGE +'/media/'+ str(capacitacion.archivoconvenio)).replace('\\', '/')
                    ...
                    with open(url_archivo, 'rb') as f:
                        archivo_a_firmar = f.read()

                    for membrete in txtFirmas:
                        datau = JavaFirmaEc(
                    ...
                    file_obj = DjangoFile(archivo_a_firmar, name=f"{remover_caracteres_especiales_unicode(_name)}.pdf")
                    capacitacion.archivoconvenio = file_obj
                    ...
                    vice = capacitacion.obtenerdatosautoridad('VICE', periodo)
                    if vice:
                        send_html_mail("Solicitud de capacitación",
                                       ...
                                       {'sistema': u'SGA - UNEMI',
                                        ...
                                        'autoridad1': vice.persona,
                                        'autoridad2': '',
                                        't': miinstitucion()
                                        },
                                       vice.persona.lista_emails_envio(),
                                       [],
                                       cuenta=variable_valor('CUENTAS_CORREOS')[0]
                                       )
                    #return JsonResponse({"result": False, "mensaje": "Guardado con exito", }, safe=False)
                    return HttpResponseRedirect(f'/pro_cronograma?action=planificarcapacitaciones&convocatoria={capacitacion.cronograma_id}')
                except Exception as ex:
                    transaction.set_rollback(True)
                    #return JsonResponse({"result": True, "mensaje": u"Error al guardar. %s" % ex.__str__()}, safe=False)
                    return HttpResponseRedirect(
                        f'/pro_cronograma?action=planificarcapacitaciones&convocatoria={capacitacion.cronograma_id}&info=SysError: Error al guardar. {ex}')
...
#                    return JsonResponse({"result": True, "mensaje": u"Error al guardar. %s" % ex.__str__()}, safe=False)
            ...
            if action == 'segmento':
                try:
                    ...
                    template = get_template("pro_cronograma/segmento.html")
                    json_content = template.render(data)
                ...
                try:
                    ...
                    if f.is_valid():
                        ...
                        if evidenciaactividaddetalledistributivo.criterio.criterioinvestigacionperiodo:
                            ...
                            migrar_evidencia_integrante_proyecto_externo(request=request, evidencia=evidenciaactividaddetalledistributivo)
            ...
            if action == 'firmarinformepppinternadorotativo':
                ...
                try:
                    ...
                    return JsonResponse({"result": 'ok', 'r': f'/pro_cronograma?action=verevidencia&id={encrypt(evidenciaactividad.criterio.pk)}'})
                except Exception as ex:
                ...
                try:
                    ...
                    estadoinicio = int(evidencia.estadoaprobacion)
                    f = EvidenciaActividadDetalleDistributivoForm(request.POST)
                    newfile = None
                    if 'archivo' in request.FILES:
                    ...
                    if f.is_valid():
                        ...
                        if EvidenciaActividadDetalleDistributivo.objects.filter(subactividad=evidencia.subactividad, criterio=evidencia.criterio, hasta__month=f.cleaned_data['hasta'].month, hasta__year=f.cleaned_data['hasta'].year, status=True).exclude(pk=evidencia.pk).exists():
                            return JsonResponse({"result": "bad", "mensaje": u"Esta actividad solo permite cargar una evidencia por mes, que corresponde al informe mensual de la actividad, los archivos adicionales deben subirse como anexo del informe del mes correspondiente."})

                        evidencia.desde = f.cleaned_data['desde']
                        evidencia.hasta = f.cleaned_data['hasta']
                        evidencia.actividad = f.cleaned_data['actividad']

                        if newfile:
                            evidencia.archivo = newfile
                            evidencia.archivofirmado = None if estadoinicio in (1, 3) else newfile

                        if estadoinicio == 3:
                            if newfile:
                                personarevisor = None
                                if doc := evidencia.criterio.criteriodocenciaperiodo:
                                    if obj := doc.usercriteriorevisor_set.filter(tiporevisor=1, status=True).first():
                                        personarevisor = obj.persona

                                if inv := evidencia.criterio.criterioinvestigacionperiodo:
                                    if obj := inv.usercriteriorevisor_set.filter(tiporevisor=1, status=True).first():
                                        personarevisor = obj.persona

                                if ges := evidencia.criterio.criteriogestionperiodo:
                                    if obj := ges.usercriteriorevisor_set.filter(tiporevisor=1, status=True).first():
                                        personarevisor = obj.persona
                            ...
                            historial = HistorialAprobacionEvidenciaActividad(aprobacionpersona=persona, fechaaprobacion=datetime.now().date(), observacion='Edición de la evidencia', evidencia=evidencia, estadoaprobacion=1)
                            historial.save(request)

                        if evidencia.criterio.criteriodocenciaperiodo and evidencia.criterio.criteriodocenciaperiodo.criterio.pk == TUTOR_PRACTICAS_INTERNADO_ROTATIVO:
                            if request.POST.get('conf'):
                                if conf := ConfiguracionInformePracticasPreprofesionales.objects.filter(pk=request.POST['conf']).first():
                                    generar_informe_practicas_preprofesionales(request=request, evidencia=evidencia, configuracion=conf)

                        evidencia.save(request)

                        if newfile and evidencia.archivo:
                            evidenciaaudi = EvidenciaActividadAudi(evidencia=evidencia, archivo=f"{evidencia.archivo.url}")
                            evidenciaaudi.save(request)
                        ...
                        log(f'Modifico Evidencia Profesor Distributivo Horas: {evidencia}', request, "edit")
                        return JsonResponse({"result": "ok", 'id_evidencia': evidencia.pk})
                    else:
                ...
                try:
                    ...
                    evidenciaactividad.save(request)
                    esdecanofacultad = personadirectorcoordinacion.persona == persona and periodo.pk not in (3, 4)
                    ...
                    if not HistorialInforme.objects.values('id').filter(informe=evidenciaactividad, estado=3).exists():
                        extra = {'firmado':True, 'fechafirma':datetime.now().date()} if esdecanofacultad else {}
                        historial = HistorialInforme(informe=evidenciaactividad,
                    ...
                    if not HistorialInforme.objects.values('id').filter(informe=evidenciaactividad, estado=4).exists():
                        decanofacultad = personadirectorcoordinacion.persona
                        if esdecanofacultad:
                            if vicerectoraacademico := DistributivoPersona.objects.filter(denominacionpuesto=variable_valor('ID_CARGO_VICERRECTOR_ACADEMICO'), estadopuesto=1, status=True).first():
                                decanofacultad = vicerectoraacademico.persona
                        historial = HistorialInforme(informe=evidenciaactividad,
                                                     personafirmas=decanofacultad,
                                                     estado=4)
                        historial.save(request)
            ...
            if action == 'delpersonabitacora':
                ...
                try:
                    ...
                    if objmigracion := MigracionEvidenciaActividad.objects.filter(evidencia=registro).first():
                        ...
                        if configuracion := objmigracion.informevinculacion:
                            for act in configuracion.actividadextravinculacion_set.filter(status=True):
                                act.status = False
                                act.save(request)
                ...
                try:
                    ...
                    template = get_template("pro_cronograma/detallelaboratorio.html")
                    json_content = template.render(data)
                    return JsonResponse({"result": "ok", 'html': json_content})
                except Exception as ex:
                    return JsonResponse({"result": "bad", "mensaje": u"Error al obtener los datos."})
                ...
                try:
                    ...
                    for x in facultad:
                        facultad = x.nombre
                    return conviert_html_to_pdf('pro_cronograma/reporteindividual_pdf.html',
                ...
                try:
                    ...
                    for x in coordina:
                        facultad = x.nombre
                    return conviert_html_to_pdf('pro_cronograma/reportemasivo_pdf.html',
                                                {'pagesize': 'A4 landscape',
                ...
                try:
                    ...
                    template = get_template("pro_cronograma/grupodocentes.html")
                    json_content = template.render(data)
                    return JsonResponse({"result": "ok", 'data': json_content})
                except Exception as ex:
                    return JsonResponse({"result": "bad"})
                ...
                try:
                    ...
                    data['asignatura'] = asignaturamallapreferencia.asignaturamalla.asignatura
                    data['title'] = u'Docentes'
                    template = get_template("pro_cronograma/grupodocentesposgrado.html")
                    json_content = template.render(data)
                    return JsonResponse({"result": "ok", 'data': json_content})
                except Exception as ex:
                    return JsonResponse({"result": "bad"})
                ...
                try:
                    ...
                    data['title'] = u'Docentes'
                    template = get_template("pro_cronograma/grupoinvestigacion.html")
                    json_content = template.render(data)
                    return JsonResponse({"result": "ok", 'data': json_content})
                except Exception as ex:
                    return JsonResponse({"result": "bad"})
                ...
                try:
                    ...
                    data['title'] = u'Docentes'
                    template = get_template("pro_cronograma/preferenciacriteriodocentes.html")
                    json_content = template.render(data)
                    return JsonResponse({"result": "ok", 'data': json_content})
                except Exception as ex:
                    return JsonResponse({"result": "bad"})
                ...
                try:
                    ...
                    template = get_template("pro_cronograma/preferenciacriterioinvestigacion.html")
                    json_content = template.render(data)
                    return JsonResponse({"result": "ok", 'data': json_content})
                except Exception as ex:
                    return JsonResponse({"result": "bad"})
                ...
                try:
                    ...
                    template = get_template("pro_cronograma/preferenciacriteriogestion.html")
                    json_content = template.render(data)
                    return JsonResponse({"result": "ok", 'data': json_content})
                except Exception as ex:
                    return JsonResponse({"result": "bad"})
                ...
                try:
                    ...
                    template = get_template("pro_cronograma/historicoasignaturas.html")
                    json_content = template.render(data)
                    return JsonResponse({"result": "ok", 'data': json_content})
                except Exception as ex:
                    return JsonResponse({"result": "bad"})
            ...
            #         return JsonResponse({"result": "bad"})
                ...
                try:
                    ...
                    return HttpResponseRedirect(f"/pro_cronograma?info=[{sys.exc_info()[-1].tb_lineno}] Problemas al generar el informe. {ex.__str__()}")
                ...
                try:
                    ...
                    if int(request.POST['tipopersona']) == 1:
                        if not BitacoraActividadDiaria.objects.values('id').filter(filtro).exists():
                            return HttpResponseRedirect(f"/pro_cronograma?info={mensaje % 'tutor'}")
                        eDatainfo = profesor.informe_actividades_mensual_tutor_posgrado(periodo, fechainicio, fechafin, Materia.objects.get(pk=idmate) if idmate else None)
                        return download_html_to_pdf('adm_criteriosactividadesdocente/informe_actividad_diaria_tutor_posgrado.html',{'pagesize': 'A4','data': eDatainfo})
                    else:
                        if not BitacoraActividadDiaria.objects.values('id').filter(filtro).exists():
                            return HttpResponseRedirect(f"/pro_cronograma?info={mensaje % 'coordinador'}")
                        ...
                        eSolicitudPago = SolicitudPago.objects.values('id').filter(status=True,
                                                           fechainicio__date=finicio,
                                                           fechaifin__date=ffin,
                                                           contrato=contrato)
                        if eSolicitudPago.exists():
                            return HttpResponseRedirect(f"/pro_solicitudpago?action=requisitos_solicitudes_pagos&id={encrypt(eSolicitudPago[0]['id'])}")
                        ...
                        eedatainfo=profesor.informe_actividades_mensual_coordinador_posgrado(periodo, fechainicio,fechafin)
                        resp = conviert_html_to_pdf_name_bitacora('adm_criteriosactividadesdocente/informe_actividad_diaria_coordinador_posgrado.html', {"data": eedatainfo},name_file)
                        ...
                        solicitud.horas_ejecutadas_coordinador = eedatainfo['totalhorasgestionejecutadas']
                        solicitud.save(request)
                    ...
                    return HttpResponseRedirect("/pro_cronograma?info=Problemas al generar el informe. Error: %s" % ex.__str__())
                ...
                try:
                    ...
                    if periodo.periodo_academia() and not periodo.versionreporte == 4:
                        if periodo.periodo_academia().periodos_relacionados:
                            ...
                            return conviert_html_to_pdf('adm_criteriosactividadesdocente/informe_fuera_periodo.html', {'pagesize': 'A4',
                    ...
                    if periodo.versionreporte == 1:
                        return conviert_html_to_pdf('adm_criteriosactividadesdocente/informe_actividad_docente_pdf.html', {'pagesize': 'A4', 'data': profesor.informe_actividades_mensual_docente(periodo, request.POST['fini'], request.POST['ffin'], 'FACULTAD')})
                    if periodo.versionreporte == 2:
                        return conviert_html_to_pdf('adm_criteriosactividadesdocente/informe_actividad_docente_new_pdf.html', {'pagesize': 'A4', 'data': profesor.informe_actividades_mensual_docente_new(periodo, request.POST['fini'], request.POST['ffin'], 'FACULTAD')})
                    if periodo.versionreporte == 3:
                        return conviert_html_to_pdf('adm_criteriosactividadesdocente/informe_actividad_docentev3_pdf.html', {'pagesize': 'A4', 'data': profesor.informe_actividades_mensual_docente_new(periodo, request.POST['fini'], request.POST['ffin'], 'FACULTAD')})
                    if periodo.versionreporte == 4:
                        return download_html_to_pdf('adm_criteriosactividadesdocente/informe_actividad_docentev4_pdf.html', {'pagesize': 'A4', 'data': profesor.informe_actividades_mensual_docente_v4(periodo, request.POST['fini'], request.POST['ffin'], 'FACULTAD', request.POST['contenidopromedio'])})

                except Exception as ex:
                    return HttpResponseRedirect("/pro_cronograma?info=%s" % ex)

            elif action == 'informedocenteadm':
                mensaje = "Problemas al generar el informe de actividades admision."
                try:
                    return conviert_html_to_pdf('adm_criteriosactividadesdocente/informe_actividad_docente_pdfadm.html', {'pagesize': 'A4', 'data': profesor.informe_actividades_mensual_docente(periodo, request.POST['fini'], request.POST['ffin'], 'ADMISION')})
                except Exception as ex:
                ...
                try:
                    ...
                    if opcionreport == 2 or opcionreport == 3:
                        coord = 9
                    return conviert_html_to_pdf('pro_cronograma/reporte_mensajes.html', {'pagesize': 'A4', 'data': {'profesor': profesor, 'coord': coord}})
                except Exception as ex:
                    return HttpResponseRedirect("/pro_cronograma?info=%s" % mensaje)
                ...
                try:
                    ...
                    return conviert_html_to_pdf('pro_cronograma/reporte_foros.html', {'pagesize': 'A4', 'data': {'profesor': profesor, 'coord': coord, 'materias': materias}})
                except Exception as ex:
                    return HttpResponseRedirect("/pro_cronograma?info=%s" % mensaje)
                ...
                try:
                    ...
                    if opcionreport == 3:
                        ...
                        resultado.append((asistencias_reg, asistencias_no_registradas, porcentaje))
                    if periodo.versionreporte == 2:
                        return conviert_html_to_pdf('pro_cronograma/informe_seguimiento_new.html', {'pagesize': 'A4',
                                                                                                    'data': {'distributivo': distributivo,
                                                                                                             'periodo': periodo,
                                                                                                             'fini': finio,
                                                                                                             'ffin': ffino,
                    ...
                    if periodo.versionreporte == 3:
                        ...
                        return conviert_html_to_pdf('pro_cronograma/informe_seguimiento_newv3.html', {'pagesize': 'A4',
                                                                                                      'data': {
                    ...
                    return HttpResponseRedirect("/pro_cronograma?info=%s - %s" % (mensaje, ex.__str__()))
                ...
                try:
                    ...
                    numero_solicitudes_cerrado = SolicitudTutorSoporteMateria.objects.filter(fecha_creacion__range=(finic, ffinc), status=True, materiaasignada__matricula__estado_matricula__in=[2, 3], materiaasignada__matricula__nivel__periodo=periodo, profesor=profesor, estado=3).distinct().count()
                    numero_solicitudes_solicitado = SolicitudTutorSoporteMateria.objects.filter(fecha_creacion__range=(finic, ffinc), status=True, materiaasignada__matricula__estado_matricula__in=[2, 3], materiaasignada__matricula__nivel__periodo=periodo, profesor=profesor, estado=1).distinct().count()
                    return download_html_to_pdf('pro_cronograma/informe_tutor_posgrado.html', {'pagesize': 'A4',
                                                                                               'data': {
                                                                                                   'distributivo': distributivo,
                                                                                                   'periodo': periodo,
                                                                                                   'fini': finio,
                ...
                try:
                    ...
                    return conviert_html_to_pdf('pro_cronograma/informe_seguimiento_virtual.html',
                                                {'pagesize': 'A4',
                ...
                try:
                    ...
                    return conviert_html_to_pdf(
                        'pro_cronograma/controlacademico_pdf.html',
                        {
                            'pagesize': 'A4',
                            'data': data,
                        }
                    )
                    # return conviert_html_to_pdf('pro_cronograma/informeestudiantespracticas.html',{'pagesize': 'A4 landscape', 'datos': data})
                except Exception as ex:
                    return JsonResponse({"result": "bad", "mensaje": u"Error al guardar los datos."})
                ...
                try:
                    ...
                    return conviert_html_to_pdf('pro_cronograma/informe_seguimiento_practica_pdf.html', {'pagesize': 'A4 landscape', 'datos': data})
                except Exception as ex:
                    return HttpResponseRedirect("/pro_cronograma?info=%s" % 'Error al generar informe de estudiantes de prácticas')
                ...
                try:
                    ...
                    template = get_template("pro_cronograma/segmento_practica.html")
                    json_content = template.render(data)
                ...
                try:
                    ...
                    template = get_template("pro_cronograma/addvisitasupervisor.html")
                    json_content = template.render(data)
                ...
                try:
                    ...
                    template = get_template("pro_cronograma/editvisitasupervisor.html")
                    json_content = template.render(data)
                    return JsonResponse({"result": "ok", 'titulo': 'Agendar visita de prácticas asignadas', "data": json_content})
                except Exception as ex:
                    return JsonResponse({"result": "bad", "mensaje": u"Error al consultar los datos."})
                ...
                try:
                    ...
                    data['ESTADO_TIPO_VISITA'] = ESTADO_TIPO_VISITA
                    template = get_template("pro_cronograma/listavisitasupervisor.html")
                    json_content = template.render(data)
                ...
                try:
                    ...
                    template = get_template("pro_cronograma/obsvisitasupervisor.html")
                    json_content = template.render(data)
                ...
                try:
                    ...
                    return conviert_html_to_pdf('pro_cronograma/informe_vinculacion_practica_pdf.html', {'pagesize': 'A4 landscape', 'datos': data})
                except Exception as ex:
                    return HttpResponseRedirect("/pro_cronograma?action=visitasupervisor&info=%s" % 'Error al generar informe de vinculación de prácticas')

            elif action == 'informefichaseguimientopractica':
                try:
                    data['visitapractica'] = VisitaPractica.objects.get(pk=int(encrypt(request.POST['id'])), persona=persona)
                    return conviert_html_to_pdf('pro_cronograma/informe_ficha_seguimiento_practica_pdf.html', {'pagesize': 'A4 landscape', 'datos': data})
                except Exception as ex:
                    return HttpResponseRedirect("/pro_cronograma?action=visitasupervisor&info=%s" % 'Error al generar informe de vinculación de prácticas')
                ...
                try:
                    ...
                    template = get_template("pro_cronograma/detalleaprobacionsilabo.html")
                    json_content = template.render(data)
                    return JsonResponse({"result": "ok", 'data': json_content})
                except Exception as ex:
                    return JsonResponse({"result": "bad", "mensaje": u"Error al obtener los datos."})
                ...
                try:
                    ...
                    template = get_template("pro_cronograma/seguimientosilabo.html")
                    json_content = template.render(data)
                    return JsonResponse({"result": "ok", 'data': json_content})
                except Exception as ex:
                    return JsonResponse({"result": "bad", "mensaje": u"Error al obtener los datos."})
                ...
                try:
                    ...
                    template = get_template("pro_cronograma/detalleobservacion.html")
                    json_content = template.render(data)
                    return JsonResponse({"result": "ok", 'data': json_content})
                except Exception as ex:
                    return JsonResponse({"result": "bad", "mensaje": "Error al guardar los datos."})
                ...
                try:
                    ...
                    if form.is_valid():
                        ...
                        #                 'solicitud': planificarponencias
                        #                 },
                        #                lista_email_envio,
                        #                ['ivan_saltos_medina@hotmail.com'],
                        #                [],
                ...
                try:
                    ...
                    if form.is_valid():
                        ...
                        if coordinador and decano and vice:
                            if coordinador.persona_id == profesor_id:
                ...
                try:
                    ...
                    valida = convert_html_to_pdf(
                        'pro_cronograma/cartacompromisoponenciapdf.html',
                ...
                try:
                    ...
                    template = get_template("pro_cronograma/mostrarmalla.html")
                    json_content = template.render(data)
                ...
                try:
                    ...
                    data['nivelesdemallas'] = NivelMalla.objects.filter(status=True, pk__in=AsignaturaMalla.objects.values_list('nivelmalla').filter(malla=malla).exclude(pk__in=materiaspreferencias)).order_by('id')
                    data['asignaturasmallas'] = AsignaturaMalla.objects.filter(malla=malla).exclude(pk__in=materiaspreferencias)
                    template = get_template("pro_cronograma/mostrarmallaposgrado.html")
                    json_content = template.render(data)
                    return JsonResponse({"result": "ok", 'malla': json_content})
                except Exception as ex:
                    return JsonResponse({"result": "bad", "mensaje": "Error al guardar los datos."})
                ...
                try:
                    ...
                    template = get_template("pro_cronograma/mostrarseleccionadas.html")
                    json_content = template.render(data)
                    return JsonResponse({"result": "ok", 'malla': json_content})
                except Exception as ex:
                    return JsonResponse({"result": "bad", "mensaje": "Error al guardar los datos."})
                ...
                try:
                    ...
                    data['totalpreferencia'] = materiaspreferencias.count()
                    template = get_template("pro_cronograma/mostrarseleccionadasposgrado.html")
                    json_content = template.render(data)
                    return JsonResponse({"result": "ok", 'malla': json_content})
                except Exception as ex:
                    return JsonResponse({"result": "bad", "mensaje": "Error al guardar los datos."})
                ...
                try:
                    ...
                    template = get_template("pro_cronograma/modaltutorias.html")
                    json_content = template.render(data)
                ...
                try:
                    ...
                    template = get_template("pro_cronograma/modaltutoriasedit.html")
                    json_content = template.render(data)
                    return JsonResponse({"result": "ok", 'titulo': 'Lista de tutorias', "data": json_content})
                except Exception as ex:
                    return JsonResponse({"result": "bad", "mensaje": u"Error al consultar los datos."})
                    ...
                    with transaction.atomic():
                        ...
                        if grupo_centro_estadistico:
                            ...
                            send_html_mail(titulo,
                                           "pro_cronograma/emails/notificacionsolicitudemail.html",
                ...
                try:
                    with transaction.atomic():
                        ...
                        if grupo_centro_estadistico:
                            ...
                            send_html_mail(titulo,
                                           "pro_cronograma/emails/notificacionsolicitudmodificadaemail.html",
                                           {'sistema': request.session['nombresistema'], 'registro': solicitud,
            ...
            if action == 'delasesoramientossee':
                try:
                    ...
                    if grupo_centro_estadistico:
                        ...
                        send_html_mail(titulo,
                                       "pro_cronograma/emails/notificacionsolicitudeliminadaemail.html",
            ...
            if action == 'addbitacora':
                try:
                    persona, estadoaprobacion = request.session['persona'], 1
                    ...
                    if bitacoradocente.subactividad and not bitacoradocente.subactividad.subactividaddocenteperiodo.validacion:
                        estadoaprobacion = 2
            ...
            if action == 'solicitarrevisionbitacora':
                try:
                    ...
                    bitacora.registrotardio = False
                    bitacora.save(request, update_fields=['estadorevision', 'registrotardio'])
        ...
        if 'action' in request.GET:
            ...
            if action == 'legalizarterminoscondiciones':
                try:
                    ...
                    template = get_template("pro_cronograma/modal/firmarinformepppinternadorotativo.html")
                    return JsonResponse({"result": True, 'data': template.render(data)})
                except Exception as ex:
                    transaction.set_rollback(True)
                    return JsonResponse({"result": "bad", "mensaje": u"Error al obtener los datos."})
            ...
            if action == 'culminartutoria':
                try:
                    ...
                    return render(request, "pro_cronograma/culminartutoria.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/listadotutorias.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    query = AgendaPracticasTutoria.objects.filter(docente=perfilprincipal.profesor, status=True).order_by('-fecha')
                    data['listado'] = query
                    return render(request, "pro_cronograma/programartutoria.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['form2'] = form
                    template = get_template("pro_cronograma/modal/formagenda.html")
                    return JsonResponse({"result": True, 'data': template.render(data)})
                except Exception as ex:
                    print('Error on line {}'.format(sys.exc_info()[-1].tb_lineno))
                ...
                try:
                    ...
                    data['form2'] = form
                    template = get_template("pro_cronograma/modal/veragenda.html")
                    return JsonResponse({"result": True, 'data': template.render(data)})
                except Exception as ex:
                    print('Error on line {}'.format(sys.exc_info()[-1].tb_lineno))
                ...
                try:
                    ...
                    data['form2'] = form
                    template = get_template("pro_cronograma/modal/formobservacion.html")
                    return JsonResponse({"result": True, 'data': template.render(data)})
                except Exception as ex:
                    print('Error on line {}'.format(sys.exc_info()[-1].tb_lineno))
                ...
                try:
                    ...
                    data['form2'] = form
                    template = get_template("pro_cronograma/modal/formfinalizacion.html")
                    return JsonResponse({"result": True, 'data': template.render(data)})
                except Exception as ex:
                    print('Error on line {}'.format(sys.exc_info()[-1].tb_lineno))
                ...
                try:
                    ...
                    valida = conviert_html_to_pdfsaveqrinformepracticasmensual(
                        'pro_cronograma/informemensualpdf.html',
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/calificacionact.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/listaasignaturas.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/preferencias.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/preferenciasactividad.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/preferenciaposgrado.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/prehorario.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['form'] = form
                    return render(request, "pro_cronograma/addpractica.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/editarpractica.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/delpractica.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    participante.delete()
                    return HttpResponseRedirect("pro_cronograma?action=calificarpractica&id=" + str(practica.id))
                except Exception as ex:
                    transaction.set_rollback(True)
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/retirarpry.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['hoy'] = datetime.now().date()
                    return render(request, "pro_cronograma/practicas.html", data)
                except Exception as ex:
                    pass
            ...
            #         return render(request, "pro_cronograma/practicamateria.html", data)
            #     except Exception as ex:
            #         pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/calificarpractica.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/calificacionpry.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['search'] = search if search else ""
                    return render(request, "pro_cronograma/calificacioncurso.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['form'] = form
                    return render(request, "pro_cronograma/subirevidencia.html", data)
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/editevidencia.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['formanexo'] = form
                    template = get_template("pro_cronograma/addanexoevidencia.html")
                    return JsonResponse({"result": True, 'data': template.render(data)})
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    form = InformeForm()
                    data['form'] = form
                    template = get_template("pro_cronograma/addinforme.html")
                    return JsonResponse({"result": True, 'data': template.render(data)})
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    can_upload_evidence, can_delete_evidence, can_update_evidence = True, False, False
                    ...
                    if doc := detalledistributivo.criteriodocenciaperiodo:
                        ...
                        can_update_evidence = doc.criterio.pk == TUTOR_PRACTICAS_INTERNADO_ROTATIVO
                    ...
                    fechainicio, fechafin = periodo.inicio, periodo.fin
                    if actividad := detalledistributivo.actividaddetalledistributivo_set.filter(vigente=True, status=True).first():
                        fechainicio, fechafin = actividad.desde, actividad.hasta

                    for i, fecha in enumerate(list(rrule(MONTHLY, dtstart=fechainicio, until=fechafin))):
                        ...
                        if (lastmonthday - fecha.day) <= 5 and i == 0:
                            ...
                            replicaevidenciaactividaddetalledistributivo = evidenciaactividaddetalledistributivo
                            if subactividad and subactividad.subactividaddocenteperiodo.criterio.pk == CRITERIO_PROYECTO_INVESTIGACION_CONVENIO:
                                replicaevidenciaactividaddetalledistributivo = evidenciaactividaddetalledistributivo.exclude(migracionevidenciaactividad__proyectoinvestigacion__convocatoria__tipo=2).distinct()
                            for bita in replicaevidenciaactividaddetalledistributivo:
                    ...
                    data['puede_editar_evidencia'] = can_update_evidence
                    data['puede_eliminar_evidencia'] = can_delete_evidence
                    data['evidenciaactividaddetalledistributivo'] = evidenciaactividaddetalledistributivo
                    data['CRITERIO_PAR_EVALUADOR'] = CRITERIO_PAR_EVALUADOR
                    return render(request, "pro_cronograma/verevidencia.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    listadomeses, fechas_mensuales, now = [], [], datetime.now().date()
                    dias_plazo_llenar_bitacora = variable_valor('DIA_LIMITE_INFORME_MENSUAL')
                    ...
                    if es_actividad_macro:
                        ...
                        fechainicio, fechafin = subactividad.fechainicio, subactividad.fechafin
                    else:
                        if actividad := detalledistributivo.actividaddetalledistributivo_set.values_list('desde', 'hasta').filter(vigente=True, status=True).first():
                            fechainicio, fechafin = actividad
                        elif actividad := detalledistributivo.actividaddetalledistributivo_set.values_list('desde', 'hasta').filter(status=True).order_by('-hasta').first():
                            fechainicio, fechafin = actividad


                    fechas_mensuales = list(rrule(MONTHLY, dtstart=fechainicio, until=fechafin))
                    ...
                    for fecha in fechas_mensuales:
                        primer_dia = fecha.replace(day=1).date()
                        ultimo_dia = fecha.replace(day=calendar.monthrange(fecha.year, fecha.month)[1]).date()
                        numeromes = fecha.month
                        anio = fecha.year
                        nomostrar = len([1 for bita in registrobitacoras if bita.fechaini.month == fecha.month])

                        if primer_dia <= fechainicio <= (primer_dia + timedelta(4)):
                            nomostrar = 1

                        if nomostrar == 0 and fechaactual >= primer_dia:
                            if ultimo_dia > fechabitacora:
                                listadomeses.append([primer_dia, anio, '01', str(numeromes), str(ultimo_dia.day), ultimo_dia + timedelta(days=dias_plazo_llenar_bitacora)])

                    data['periodo'] = periodo
                    data['fechaactual'] = fechaactual
                    data['listadomeses'] = listadomeses
                    data['detalledistributivo'] = detalledistributivo
                    data['bitacoras'] = registrobitacoras.filter(fechamaxima__lte=datetime.now(), fecha_creacion__gte=fechabitacora).distinct()
                    return render(request, "pro_cronograma/listadobitacora.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    mesbitacora, subactividad = None, request.GET.get('ids')
                    if idbitacora := request.GET.get('idbitacora'):
                        mesbitacora = BitacoraActividadDocente.objects.get(pk=int(encrypt(idbitacora)))
                    else:
                        y, m = int(request.GET['y']), int(request.GET['m'])
                        fechaini, fechafin = datetime(y, m, 1, 0, 0), datetime(y, m, calendar.monthrange(y, m)[1], 0, 0)
                        detalledistributivo = DetalleDistributivo.objects.get(id=int(encrypt(request.GET['id'])))
                        mesbitacora = BitacoraActividadDocente.objects.filter(subactividad_id=subactividad, profesor=profesor, criterio=detalledistributivo, fechaini=fechaini, fechafin=fechafin).first()
                        if not mesbitacora:
                            mesbitacora = BitacoraActividadDocente(subactividad_id=subactividad, profesor=profesor, criterio=detalledistributivo, nombre=f"REGISTRO DE BITÁCORA MES DE: {MESES_CHOICES[fechafin.month - 1][1]} {fechafin.year}".upper(), fechaini=fechaini, fechafin=fechafin)
                        if subactividad and not mesbitacora.subactividad.subactividaddocenteperiodo.validacion:
                            mesbitacora.estadorevision = 3
                    ...
                    if variable_valor('VALIDA_REGISTRO_BITACORA_PPPIR'):
                        ...
                        if not mesbitacora.subactividad:
                            puede_modificar_bitacora &= mesbitacora.estadorevision == 1

                    # data['listadodetalle'] = mesbitacora.detallebitacoradocente_set.filter(status=True).annotate(diferencia_horas=Extract('horafin', 'hour') - Extract('horainicio', 'hour'), diferencia_minutos=Extract('horafin', 'minute') - Extract('horainicio', 'minute')).annotate(cantidad_horas=(F('diferencia_horas') + F('diferencia_minutos') / 60))

                    listadodetalle = mesbitacora.detallebitacoradocente_set.filter(status=True).annotate(diferencia=ExpressionWrapper(F('horafin') - F('horainicio'), output_field=TimeField())).order_by('fecha', 'horainicio', 'horafin')
                    totalhorasplanificadas, totalhorasregistradas, totalhorasaprobadas, porcentaje_cumplimiento = mesbitacora.get_horasplanificadas(), mesbitacora.get_horasregistradas(), 0, 0
                    ...
                    if mesbitacora.registrotardio and mesbitacora.estadorevision == 1:
                        puede_modificar_bitacora, puede_enviar_a_revision = True, True
                    ...
                    nextinput = None
                    if lastinput := mesbitacora.detallebitacoradocente_set.filter(status=True).order_by('fecha').last():
                        nextinput = lastinput.fecha + timedelta(1)
                        if not nextinput.month == mesbitacora.fechafin.month:
                            nextinput = mesbitacora.fechafin

                    data['nextinput'] = nextinput
                    data['mesbitacora'] = mesbitacora
                    data['listadodetalle'] = listadodetalle
                    data['totalhorasaprobadas'] = totalhorasaprobadas
                    data['detalledistributivo'] = mesbitacora.criterio
                    data['idsubactividad'] = mesbitacora.subactividad_id
                    ...
                    data['puede_modificar_bitacora'] = puede_modificar_bitacora
                    data['valida_registro_tardio_bitacora'] = valida_registro_tardio_bitacora
                    return render(request, "pro_cronograma/detallebitacora.html", data)
                except Exception as ex:
                ...
                try:
                    ...
                    template = get_template("pro_cronograma/detalleregistrobitacora.html")
                    json_content = template.render(data)
                    return JsonResponse({"result": "ok", 'data': json_content})
                except Exception as ex:
                    transaction.set_rollback(True)
                ...
                try:
                    ...
                    template = get_template("pro_cronograma/addpersonabitacora.html")
                    return JsonResponse({"result": True, 'data': template.render(data)})
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/addbitacora.html", data)
                    # return JsonResponse({"result": True, 'data': template.render(data)})
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    if form.fields.get('persona'):
                        form.fields['persona'].queryset = Persona.objects.none()
                        form.ocultarcampos()
                    tipo, criterioperiodo = detallebitacora.bitacoradocente.get_criterio_asociado()
                    if not (tipo == 2 and criterioperiodo.criterio.pk == CRITERIO_ELABORA_ARTICULO_CIENTIFICO):
                        del form.fields['titulo']
                    data['form'] = form
                    data['anexos'] = detallebitacora.anexodetallebitacoradocente_set.filter(status=True)
                    return render(request, "pro_cronograma/editbitacora.html", data)
                except Exception as ex:
                    pass
            ...
            if action == 'buscarpersonas':
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/subirconograma.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/deleteevidencia.html", data)
                except:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/aprobarevidenciasactividades.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/aprobar.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/cronograma.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/listadoinscritos.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    form = AvTutoriasForm()
                    data['form'] = form
                    return render(request, "pro_avtutorias/aadtutoriaclase.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['form'] = form
                    return render(request, "pro_avtutorias/edittutoriaclase.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_avtutorias/deletutoriaclase.html", data)
                except:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_avtutorias/view.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/listainscritocalificar.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    if opc == '1':
                        ...
                        return render(request, "pro_cronograma/evidenciaspracticas.html", data)
                    elif opc == '2':
                        data['title'] = u'Evidencias de Prácticas Pre Profesionales'
                        data['practicas'] = practicas = PracticasPreprofesionalesInscripcion.objects.get(pk=request.GET['id'])
                        data['evidencias'] = practicas.periodoppp.evidencias_practica()
                        ...
                        return render(request, "pro_cronograma/evidenciaspracticassupervisor.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['opc'] = 2
                    template = get_template("adm_seguimientosilabo/modal/formmodal.html")
                    # template = get_template("pro_cronograma/add_aprobarevidenciaspracticas.html")
                    json_content = template.render(data)
                ...
                try:
                    ...
                    template = get_template("adm_seguimientosilabo/modal/formmodal.html")
                    # template = get_template("pro_cronograma/aprobarrechazartutor.html")
                    json_content = template.render(data)
                    return JsonResponse({"result": "ok", 'data': json_content, 'title': data['title']})
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    template = get_template("adm_seguimientosilabo/modal/formmodal.html")
                    # template = get_template("pro_cronograma/ponerfechalimite.html")
                    json_content = template.render(data)
                    return JsonResponse({"result": "ok", 'data': json_content, 'title': data['title']})
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/listatutorias.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/seguimiento.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['listado'] = listado = InformeMensualDocentesPPP.objects.filter(status=True, persona=profesor).order_by('-fechageneracion')
                    data['liscount'] = listado.count()
                    return render(request, "pro_cronograma/evidenciainformes.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    template = get_template("pro_cronograma/actividades.html")
                    json_content = template.render(data)
                    return JsonResponse({"result": "ok", 'data': json_content})
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    template = get_template("pro_cronograma/verasistencia.html")
                    json_content = template.render(data)
                    return JsonResponse({"result": "ok", 'data': json_content})
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/listasupervision.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['form'] = form
                    return render(request, "pro_cronograma/configurarfechamasivapractica.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['form'] = form
                    return render(request, "pro_cronograma/retiro.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['search'] = search if search else ""
                    data['ids'] = ids if ids else ""
                    return render(request, "pro_cronograma/viewinformemensualsupervisor.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    form.adicionar()
                    data['form'] = form
                    return render(request, "pro_cronograma/addinformemensual.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/editinformemensual.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/delinformemensual.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['search'] = search if search else ""
                    data['ids'] = ids if ids else ""
                    return render(request, "pro_cronograma/viewinformemensualsupervisor_tutor.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['form'] = form
                    return render(request, "pro_cronograma/editpractica.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/visitapractica.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/delvisitaagendada.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/viewdetallevisita.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/supervisionmateriaprofesor.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/horarioprofesor.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['atras'] = 'pro_cronograma?action=supervisionmateriaprofesor&idm=' + encrypt(profesormateria.materia_id)
                ...
                try:
                    ...
                    data['rechazar'] = variable_valor('RECHAZAR_SILABO')
                    data['pendiente'] = variable_valor('PENDIENTE_SILABO')
                    template = get_template("pro_cronograma/seguimientosilabodigital.html")
                    json_content = template.render(data)
                    return JsonResponse({"result": "ok", 'data': json_content})
                except Exception as ex:
                    return JsonResponse({"result": "bad", "mensaje": u"Error al obtener los datos."})
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/planificarponencias.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/planificarcapacitaciones.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/informesgenerados.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['form'] = form
                    return render(request, "pro_cronograma/addponencia.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/addcapacitacion.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/editcapacitacion.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/subirevidenciaejecutado.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['id'] = 123
                    data['form'] = form
                    template = get_template("pro_cronograma/subirevidenciaejecutadopon.html")
                    json_content = template.render(data)
                    return JsonResponse({"result": "ok", 'data': json_content, 'title': data['title']})
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/editponencia.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    template = get_template("pro_cronograma/subircartacompromiso.html")
                    json_content = template.render(data)
                    return JsonResponse({"result": "ok", 'data': json_content, 'title': data['title']})
                except Exception as ex:
                    return JsonResponse({"result": "bad", "mensaje": u"Error al obtener los datos."})
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/deleponencia.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/delcapacitacion.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['detalledistributivo'] = DetalleDistributivo.objects.get(pk=int(encrypt(request.GET['id'])), distributivo__profesor=profesor, status=True)
                    return render(request, "pro_cronograma/productoinvestigacion.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['form'] = form
                    data['detalledistributivo'] = DetalleDistributivo.objects.get(pk=int(encrypt(request.GET['id'])), distributivo__profesor=profesor, status=True)
                    return render(request, "pro_cronograma/addproductoinvestigacionarticulo.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['form'] = form
                    data['detalledistributivo'] = DetalleDistributivo.objects.get(pk=int(encrypt(request.GET['id'])), distributivo__profesor=profesor, status=True)
                    return render(request, "pro_cronograma/addproductoinvestigacionponencia.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['form'] = form
                    data['detalledistributivo'] = DetalleDistributivo.objects.get(pk=int(encrypt(request.GET['id'])), distributivo__profesor=profesor, status=True)
                    return render(request, "pro_cronograma/addproductoinvestigacionlibro.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['form'] = form
                    data['detalledistributivo'] = DetalleDistributivo.objects.get(pk=int(encrypt(request.GET['id'])), distributivo__profesor=profesor, status=True)
                    return render(request, "pro_cronograma/addproductoinvestigacioncapitulolibro.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/editarproductoarticulo.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/editarproductoponencia.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/editarproductolibro.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/editarproductocapitulolibro.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/delproductoarticulo.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/delproductoponencia.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/delproductolibro.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/delproductocapitulolibro.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    template = get_template("pro_cronograma/detalleproductoinvestigcion.html")
                    json_content = template.render(data)
                    return JsonResponse({"result": "ok", 'data': json_content})
                except Exception as ex:
                    return JsonResponse({"result": "bad", "mensaje": u"Error al obtener los datos."})
                ...
                try:
                    ...
                    if tipo == 1:
                        ...
                        cronograma = capitulolibro.cronogramatrabajoinvestigaciondocente_set.filter(status=True)
                    data['cronograma'] = cronograma
                    return render(request, "pro_cronograma/cronogramaproducto.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/addcronogramaproducto.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/editcronogramaproducto.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    if cronograma.articulo:
                        ...
                        data['tipo'] = 4
                        data['producto'] = cronograma.capitulolibro
                    return render(request, "pro_cronograma/delcronogramaproducto.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/verevidenciadesvinculadas.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['profesor'] = profesor
                    return render(request, "pro_cronograma/aceptapreferenciaassignatura.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['profesor'] = profesor
                    return render(request, "pro_cronograma/aceptapreferenciacriterio.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['profesor'] = profesor
                    return render(request, "pro_cronograma/aceptapreferenciahorario.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['profesor'] = profesor
                    return render(request, "pro_cronograma/aceptapreferenciaasignaturaposgrado.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['form'] = form
                    return render(request, "pro_cronograma/editcapacitacionth.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['search'] = search if search else ""
                    return render(request, "pro_cronograma/listarinquietudconsultaestudiante.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return conviert_html_to_pdf(
                        'pro_cronograma/reporteinquietudconsultaestudiante.html',
                        {
                            'pagesize': 'A4',
                            'data': data,
                        }
                ...
                try:
                    ...
                    template = get_template("pro_cronograma/ver_rubrica.html")
                    json_content = template.render(data)
                    return JsonResponse({"result": "ok", 'data': json_content})
                except Exception as ex:
                    transaction.set_rollback(True)
                ...
                try:
                    ...
                    template = get_template("pro_cronograma/ver_calificaciones.html")
                    json_content = template.render(data)
                    return JsonResponse({"result": "ok", 'data': json_content})
                except Exception as ex:
                    transaction.set_rollback(True)
                ...
                try:
                    data = {}
                    data['materia'] = materia = Materia.objects.get(pk=int(encrypt(request.GET['idm'])))
                    template = get_template("pro_cronograma/ver_recursos.html")
                    json_content = template.render(data)
                    return JsonResponse({"result": "ok", 'data': json_content})
                except Exception as ex:
                    transaction.set_rollback(True)
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/verasistencias_link.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/listadocontenidos.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['listadomaterias'] = Materia.objects.filter(status=True, pk__in=profesorseleccionado.profesormateria_set.values_list('materia_id').filter(materia__nivel__periodo=periodo, activo=True, status=True))
                    return render(request, "pro_cronograma/planificacionrecurso.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['criterio'] = CriterioDocencia.objects.get(pk=int(request.GET['idcriterio']))
                    data['listadomaterias'] = Materia.objects.filter(status=True, pk__in=profesorseleccionado.profesormateria_set.values_list('materia_id').filter(materia__nivel__periodo=periodo, activo=True, status=True))
                    return render(request, "pro_cronograma/listadomateriales.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/contenidocalificado.html", data)
                except Exception as ex:
                    pass
                    ...
                    if listadomaterias:
                        ...
                        for x in matriculados:
                            if x.id != matriculados.order_by('-id')[0].id:
                                ...
                                lista1 += str(x.matricula.inscripcion.persona.idusermoodle)
                    data['lista'] = lista1
                    return render(request, "pro_cronograma/contenidocalificadoonline.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/veractividadrevisor.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    template = get_template("pro_cronograma/ver_detalleevidencia.html")
                    json_content = template.render(data)
                    return JsonResponse({"result": "ok", 'data': json_content})
                except Exception as ex:
                    transaction.set_rollback(True)
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/viewtutorias.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/viewregistrotutorias.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/configuraciones.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/formconfiguracion.html", data)
                    # template = get_template("pro_cronograma/formconfiguracion.html")
                    # return JsonResponse({"result": True, 'data': template.render(data)})
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    return render(request, 'pro_cronograma/delconfiguracion.html', data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    data['totalplanasincronica'] = totalplanasincronica
                    data['profesor'] = profesor
                    return render(request, "pro_cronograma/detalle_clasesvideo.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    template = get_template("pro_cronograma/modal/detalleasesoramientossee.html")
                    return JsonResponse({"result": True, "data": template.render(data)})
                except Exception as ex:
                    # transaction.set_rollback(True)
                    return JsonResponse({"result": False, "mensaje": u"Error al guardar los datos. %s" % ex.__str__()})
                ...
                try:
                    ...
                    if fecha_filtro.date() >= fechaactual.date():
                        ...
                        template = get_template("pro_cronograma/modal/tabla_horariodisponibles.html")
                        return JsonResponse({"result": True, "data": template.render(data)})
                    else:
                ...
                try:
                    ...
                    template = get_template("pro_cronograma/modal/formasesoramientosee.html")
                ...
                try:
                    ...
                    # data['puede_registrar'] = True
                    return render(request, "pro_cronograma/horarioplanificacionpracticaspp.html", data)
                except Exception as ex:
                    pass
                ...
                try:
                    ...
                    template = get_template("pro_cronograma/detalletutorias.html")
                    json_content = template.render(data)
                    return JsonResponse({"result": "ok", 'data': json_content})
                except Exception as e:
                ...
                try:
                    ...
                    if periodo.tipo.id in [3,4]:
                        return render(request, "pro_cronograma/vercumplimientopos.html", data)
                    else:
                        return render(request, "pro_cronograma/vercumplimiento.html", data)
                except Exception as ex:
                    print('Error on line {}'.format(sys.exc_info()[-1].tb_lineno))
                    return HttpResponseRedirect("/pro_cronograma?info=%s" % ex)
                ...
                try:
                    ...
                    return render(request, "pro_cronograma/misterminosycondiciones.html", data)
                except Exception as ex:
                    return JsonResponse({'result':False, 'mensaje': ex.__str__()})
                ...
                try:
                    ...
                    data['form'] = form
                    template = get_template('pro_cronograma/modal/formsolicitud.html')
                    return JsonResponse({'result': True, 'data': template.render(data)})
                except Exception as ex:
                ...
                try:
                    ...
                    data_extra['totalhorasregistradas'] = bitacora.get_horasregistradas()
                    data_extra['totalhorasplanificadas'] = bitacora.get_horasplanificadas()
                    data_extra['porcentaje_cumplimiento'] = bitacora.get_porcentaje_cumplimiento()
                    return conviert_html_to_pdf_name('../templates/pro_cronograma/informe_bitacora_actividad_docente.html', data_extra, f"bitacora_{persona.usuario.username}_{bitacora.pk}.pdf")
                except Exception as ex:
                    pass
            ...
            try:
                ...
                if distributivo := ProfesorDistributivoHoras.objects.filter(profesor__persona=persona, periodo=periodo, activo=True, status=True).first():
                    ...
                    aprobar_bitacora_termino_plazo(periodo=periodo)
                    actualiza_vigencia_criterios_docente(distributivo)

                return render(request, "pro_cronograma/view.html", data)
            except Exception as ex:
                print('Error on pro_cronograma line {} {}'.format(sys.exc_info()[-1].tb_lineno, ex.__str__()))
            ...
            if lider := ProyectoInvestigacionIntegrante.objects.filter(tiporegistro__in=[1, 3, 4], persona=persona, proyecto__estado_id=37, funcion=funcion, status=True).exclude(proyecto__cerrado=True).first():
                ...
                if not inv.es_actividadmacro:
                    ...
                    if (evidencia.subactividad and evidencia.subactividad.subactividaddocenteperiodo.criterio.pk == CRITERIO_DIRECTOR_PROYECTO) or can_upload_evidence:
                        if not MigracionEvidenciaActividad.objects.values('id').filter(evidencia=evidencia, proyectoinvestigacion=lider.proyecto, status=True).exists():
                        ...
                        for integrante in lider.proyecto.integrantes_proyecto().exclude(funcion__in=exclude_):
                            ...
                            if distributivo := DetalleDistributivo.objects.filter(criterioinvestigacionperiodo__criterio__id=CRITERIO_ACTIVIDAD_MACRO, distributivo__profesor=integrante_profesor, distributivo__periodo=evidencia.criterio.distributivo.periodo, status=True).first():
                                ...
                                if subactividad := SubactividadDetalleDistributivo.objects.filter(actividaddetalledistributivo__vigente=True, actividaddetalledistributivo__criterio=distributivo, subactividaddocenteperiodo__criterio=CRITERIO, subactividaddocenteperiodo__actividad__status=True, subactividaddocenteperiodo__actividad__criterio__status=True, subactividaddocenteperiodo__status=True, subactividaddocenteperiodo__criterio__status=True, actividaddetalledistributivo__status=True, status=True).first():
...
def generar_informe_practicas_preprofesionales(**kwargs):
    try:
        ...
        if newfile := convert_html_to_pdf('../templates/pro_cronograma/informe_mensual_practicas_preprofesionales_salud.html', data, filename, os.path.join(os.path.join(SITE_STORAGE, 'media', filepath, ''))):
                ...
                if director := GrupoInvestigacionIntegrante.objects.filter(persona=persona, grupo__vigente=True, grupo__status=True, funcion=1, status=True).first():
                    ...
                    for integrante in grupo.grupoinvestigacionintegrante_set.filter(status=True).exclude(funcion=1):
                        if distributivo := DetalleDistributivo.objects.filter(criterioinvestigacionperiodo__criterio__id=CRITERIO_ACTIVIDAD_MACRO, distributivo__profesor__persona=integrante.persona, distributivo__periodo=periodo, distributivo__activo=True, status=True).first():
                            if subactividad := SubactividadDetalleDistributivo.objects.filter(actividaddetalledistributivo__vigente=True, actividaddetalledistributivo__criterio=distributivo, subactividaddocenteperiodo__criterio=CRITERIO_INTEGRANTE_GRUPOINVESTIGACION, status=True).first():
...
def genera_archivo_terminos_condiciones(request, terms):
    try:
        ...
        if convert_html_to_pdf('../templates/adm_criteriosactividadesdocente/terminos_condiciones_docente.html', data, filename, os.path.join(os.path.join(SITE_STORAGE, 'media', filepath, ''))):
...
def aprobar_bitacora_termino_plazo(**kwargs):
    try:
        ...
        if variable_valor('VALIDA_REGISTRO_BITACORA_PPPIR'):
            plazolimite = now - timedelta(int(variable_valor('PLAZO_APROBACION_AUTOMATICA_BITACORA')))
            for bitacora in BitacoraActividadDocente.objects.filter(detallebitacoradocente__status=True, criterio__distributivo__periodo=periodo, fechafin__lt=plazolimite, estadorevision__in=(1, 2), registrotardio=False, status=True).exclude(fechafin__month=now.month).distinct():
                fechaplazo = bitacora.fechafin + timedelta(int(variable_valor('PLAZO_APROBACION_AUTOMATICA_BITACORA')))
                if now >= fechaplazo.date():
...
def migrar_evidencia_integrante_proyecto_externo(**kwargs):
    try:
        request, evidencia = kwargs.pop('request'), kwargs.pop('evidencia')
        persona, periodo = request.session.get('persona'), request.session.get('periodo')
        if proyectoconvenio := ProyectoInvestigacionIntegrante.objects.filter(proyecto__convocatoria__tipo=2, tiporegistro__in=[1, 3, 4], persona=persona, proyecto__estado__in=[31, 37], funcion=1, proyecto__convocatoria__status=True, status=True).exclude(proyecto__cerrado=True).first():
            proyecto = proyectoconvenio.proyecto
            if subactividadbase := evidencia.subactividad:
                if not evidencia.migracionevidenciaactividad_set.filter(proyectoinvestigacion=proyecto, status=True).exists():
                    migracion = MigracionEvidenciaActividad(evidencia=evidencia, proyectoinvestigacion=proyecto)
                    migracion.save()
                for integrante in proyecto.proyectoinvestigacionintegrante_set.filter(status=True).exclude(funcion=1):
                    if subactividad := SubactividadDetalleDistributivo.objects.filter(subactividaddocenteperiodo=subactividadbase.subactividaddocenteperiodo, actividaddetalledistributivo__criterio__distributivo__periodo=periodo, actividaddetalledistributivo__criterio__distributivo__profesor__persona=integrante.persona, status=True).first():
                        if migracion := MigracionEvidenciaActividad.objects.filter(evidencia__subactividad=subactividad, evidenciabase=evidencia, status=True).first():
                            _evidencia = migracion.evidencia
                            _evidencia.actividaddetalledistributivo = evidencia.actividaddetalledistributivo
                            _evidencia.desde = evidencia.desde
                            _evidencia.hasta = evidencia.hasta
                            ...
                            _evidencia = EvidenciaActividadDetalleDistributivo(**_dict)
                            _evidencia.subactividad = subactividad
                            _evidencia.criterio = subactividad.actividaddetalledistributivo.criterio
                            _evidencia.save(request)
                            m = MigracionEvidenciaActividad(evidencia=_evidencia, evidenciabase=evidencia, proyectoinvestigacion=proyecto)
                            m.save(request)
                        _evidencia.anexoevidenciaactividad_set.filter(status=True).delete()
                        for anexo in evidencia.anexoevidenciaactividad_set.filter(status=True):
                            a = AnexoEvidenciaActividad(evidencia=_evidencia, observacion=anexo.observacion, archivo=anexo.archivo)
