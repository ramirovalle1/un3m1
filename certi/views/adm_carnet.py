...
def view(request):
    ...
    if not (perfilprincipal.es_administrativo()):
        return HttpResponseRedirect("/?info=Solo los perfiles de administrativos  pueden ingresar al modulo.")
    ...
    if request.method == 'POST':
        ...
        # if action == 'create':
        #     try:
        #         configuracion_carnet = ConfiguracionCarnet.objects.filter(tipo_perfil=2, tipo=1).first()
        #         distributivo = persona.mi_cargo_administrativocarnet()
        #         if not perfilprincipal.es_administrativo():
        #             raise NameError(f"Estimad{'a' if persona.sexo_id == 1 else 'o'}, no tiene asignado un perfil administrativo")
        ...
        if action == 'delete':
            try:
                ...
                delete = carne = Carnet.objects.filter(pk=int(encrypt(request.POST['id'])))
                carne.delete()
                log(u'Elimino carné administrativo: %s' % delete, request, "del")
                return JsonResponse({"result": "ok", "mensaje": "Carné administrtivo eliminado correctamente"})
            except Exception as ex:
                transaction.set_rollback(True)
        ...
        if 'action' in request.GET:
            ...
            if action == 'cargarfoto':
                try:
                    ...
                    return render(request, "adm_carnet/cargarfoto.html", data)
                except Exception as ex:
            ...
            try:
                data['title'] = u'Carné Administrativo'
                data['configuracion_carnet'] = configuracion_carnet = ConfiguracionCarnet.objects.filter(tipo_perfil=2, tipo=1).first()
                ...
                if not (persona.mi_cargo_actual()):
                    raise NameError(f"Estimad{'a' if persona.sexo_id == 1 else 'o'}, no tiene cargo asignado y no se permite carné Administrativo")
                ...
                if not persona.foto() and configuracion_carnet.puede_cargar_foto:
                    return HttpResponseRedirect("/adm_carnet?action=cargarfoto")
                ...
                data['carnet'] = carnet
                return render(request, "adm_carnet/view.html", data)
            except Exception as ex:
                data['msg_error'] = ex.__str__()
                return render(request, "adm_carnet/error.html", data)
