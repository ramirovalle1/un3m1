...
from sga.commonviews import adduserdata, prematricularmoduloespecial
from sga.models import Periodo, PeriodoPreMatriculacionModulo
    ...
    if request.method == 'POST':
        if 'action' in request.POST:
            ...
            if action == 'prematricularmodulo':
                return prematricularmoduloespecial(request, 1)
        ...
        if not periodo.prematriculacionactiva:
            if not PeriodoPreMatriculacionModulo.objects.filter(carrera=inscripcion.carrera, fecha_inicio__lte=hoy, fecha_fin__gte=hoy).exists():
                return HttpResponseRedirect("/?info=El periodo no se encuentra activo para poder prematricularse en modulos.")
            else:
                data['periodoprematricula'] = periodo = PeriodoPreMatriculacionModulo.objects.filter(carrera=inscripcion.carrera, fecha_inicio__lte=hoy, fecha_fin__gte=hoy)[0].periodo
        ...
        if inscripcion.prematriculamodulo_set.filter(periodo=periodo, tipo=1, asignaturas__id__in=[1818,1819]).exists():
        ...
        data['materiasmodulos'] = inscripcionmalla.malla.modulomalla_set.filter(asignatura__id__in=[1818,1819])
        return render(request, "alu_prematriculamoduloespecial/view.html", data)