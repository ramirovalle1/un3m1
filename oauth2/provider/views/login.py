...
from sga.models import Persona, TituloInstitucion, DeclaracionUsuario
    ...
    if request.method == 'POST':
        ...
        if 'next' in request.GET and request.GET['next']:
            ...
            if 'persona' in request.session:
                if app_client:
                    return HttpResponseRedirect("/oauth/2/provider/authorize?client_id=%s" % app_client.client_id)
                else:
                    return HttpResponseRedirect("/oauth/2/provider/logout")
...
def logout_user(request):
    logout(request)
    return HttpResponseRedirect("/oauth/2/provider/login")
