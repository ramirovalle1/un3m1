...
def coordinacion5():
    for carrera in Materia.objects.filter(status=True, nivel__periodo_id=177, cerrado=False,
                                          asignaturamalla__malla__carrera__coordinacion=5).exclude(
        modeloevaluativo_id=27).values_list('asignaturamalla__malla__carrera_id', flat=True).distinct():
        importar_cerrar_materias(carrera)

coordinacion5()
def actualizar_nivel_inscripcion_malla7():
    matriculas = Matricula.objects.filter(status=True, nivel__periodo_id=177, inscripcion__carrera_id__in=[135,188] )
...
# actualizar_nivel_inscripcion_malla7()