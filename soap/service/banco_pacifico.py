...
class ServiceCobro(Service):
    ...
    def testConnection(ctx, Test):
        try:
            template_xml = "banco_pacifico/conexion.xml"
            eDataBanco = {}
            setting = get_setting_soap(tipo=1, service_id=SERVICE_BANCO_PACIFICO_PK)
            if not setting:
    ...
    def consultarTransaccion(ctx, Consulta):
        try:
            template_xml = "banco_pacifico/consulta.xml"
            ...
            ePagoBancos = PagoBanco.objects.filter(persona=ePersona, status=True, reverso=False).distinct()
            ...
            # print(data_json)
            set_logging(LOGGING_BANCO_PACIFICO, LOGGING_LEVEL_INFO, LOGGING_FILE_WEBSERVICE, json.dumps({"consultarTransaccion": data_json}))
            return xml_content
        except Exception as ex:
            if eDataBanco:
                eDataBanco.set_TipoTransaccion("0002")
            ...
            data_json = eDataBanco.dictConsulta() if isinstance(eDataBanco, DataBanco) else {}
            # print(data_json)
            set_logging(LOGGING_BANCO_PACIFICO, LOGGING_LEVEL_ERROR, LOGGING_FILE_WEBSERVICE, json.dumps({"consultarTransaccion": data_json}))
            return xml_content
    ...
    def pagarTransaccion(ctx, Pagar):
        with transaction.atomic():
            try:
                template_xml = "banco_pacifico/pagar.xml"
                eDataBanco = DataBanco()
                setting = get_setting_soap(tipo=1, service_id=SERVICE_BANCO_PACIFICO_PK)
                if not setting:
                    eDataBanco.set_CodigoRespuesta("0047")
                ...
                if PagoBanco.objects.values("id").filter(persona=ePersona, rubro=eRubro, valor=eDataBanco.ValorAPagar, reverso=False).exists():
                ...
                # print(data_json)
                set_logging(LOGGING_BANCO_PACIFICO, LOGGING_LEVEL_INFO, LOGGING_FILE_WEBSERVICE, json.dumps({"pagarTransaccion": data_json}))
                return xml_content
            except Exception as ex:
                ...
                data_json = eDataBanco.dictPagar() if isinstance(eDataBanco, DataBanco) else {}
                # print(data_json)
                set_logging(LOGGING_BANCO_PACIFICO, LOGGING_LEVEL_ERROR, LOGGING_FILE_WEBSERVICE, json.dumps({"pagarTransaccion": data_json}))
                return xml_content
    ...
    def reversarTransaccion(ctx, Reversar):
        with transaction.atomic():
            try:
                template_xml = "banco_pacifico/reversar.xml"
                eDataBanco = DataBanco()
                setting = get_setting_soap(tipo=1, service_id=SERVICE_BANCO_PACIFICO_PK)
                if not setting:
                    eDataBanco.set_CodigoRespuesta("0047")
                ...
                ePagoBancos = PagoBanco.objects.filter(num_transaccion=eDataBanco.NumeroTransaccionReversar, config=setting)
                ...
                # print(data_json)
                set_logging(LOGGING_BANCO_PACIFICO, LOGGING_LEVEL_INFO, LOGGING_FILE_WEBSERVICE, json.dumps({"reversarTransaccion": data_json}))
                return xml_content
            except Exception as ex:
                transaction.set_rollback(True)
                if eDataBanco:
                ...
                data_json = eDataBanco.dictReversar() if isinstance(eDataBanco, DataBanco) else {}
                # print(data_json)
                set_logging(LOGGING_BANCO_PACIFICO, LOGGING_LEVEL_ERROR, LOGGING_FILE_WEBSERVICE, json.dumps({"reversarTransaccion": data_json}))
                return xml_content
...
url_banco_pacifico = csrf_exempt(DjangoView.as_view(services=[ServiceCobro], tns='web.bancopacifico.service', in_protocol=Soap11(validator='lxml'), out_protocol=Soap11()))
