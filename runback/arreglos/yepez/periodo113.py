...
url = ''
directory = os.path.join(SITE_STORAGE, 'archivos')
try:
    os.stat(directory)
except:
    os.mkdir(directory)
...
listadoperiodos = Periodo.objects.filter(pk__in=[113])
...
for lper in listadoperiodos:
    ...
    directory = os.path.join(ARCHIVO,  nombre_archivo)
    folder = os.path.join(os.path.join(SITE_STORAGE, 'archivos', ''))
