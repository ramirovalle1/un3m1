...
from poli.forms import PersonaPrimariaForm, SubirComprobanteForm
...
def view(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'editfoto':
            ...
            with transaction.atomic():
                try:
                    ...
                    historial.save()
                    log(u'Eliminó citas de servicio: %s' % instancia, request, "anularcita")
        ...
        # MIS INSCRICIONES
            ...
            try:
                reserva = ReservacionPersonaPoli.objects.get(id=encrypt_id(request.POST['id']))
                f = SubirComprobanteForm(request.POST, request.FILES, instancia=reserva)
                if not f.is_valid():
                    transaction.set_rollback(True)
                    return JsonResponse({'result': True, "form": [{k: v[0]} for k, v in f.errors.items()],
                                         "mensaje": "Error en el formulario"})
                reserva.archivo = f.cleaned_data['archivo']
                reserva.save(request)
                log(u'Subo comprobante de pago: %s' % persona, request, "edit")
                return JsonResponse({'result': False, 'mensaje': 'Guardado con éxito'})
            except Exception as ex:
                transaction.set_rollback(True)
                return JsonResponse({'result': True, 'mensaje': f'Error al guardar los datos {ex}'})
        ...
        if 'action' in request.GET:
            ...
            # COMPROBANTE DE PAGO
                ...
                try:
                    ...
                    data['reserva'] = reserva = ReservacionPersonaPoli.objects.get(pk=encrypt_id(request.GET['id']))
                    data['id'] = reserva.id
                    data['form'] = form
                    template = get_template('ajaxformmodal.html')
                    return JsonResponse({"result": True, 'data': template.render(data)})
