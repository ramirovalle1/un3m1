...
try:
    ...
    matriculas = Matricula.objects.filter(status=True, nivel__periodo=periodo, estado_matricula__in=[2,3],
                                          inscripcion__carrera__in=[103, 102, 96], inscripcion__inscripcionmalla__malla__inicio__year=2019).distinct().order_by(
        ...
        for materiaasignada in materiaasignadas:
            ...
            sql_examenes2 = """ SELECT DISTINCT test.id AS id_examen, test.name AS nombre_examen,
                                            ...
                                            """ % (str(matricula.inscripcion.persona.identificacion()),
                                                   "%ENLINEA-2020-01_" + str(materiaasignada.materia.asignatura.id) + "%")
