...
graduadossinnombrehtml = graduados.filter(namehtmltitulo__isnull=True).order_by('-id')
...
12275,
11986]
graduadosyanotificados = graduados.filter(id__in=idgraduadosyanotif)
...
graduados = Graduado.objects.filter(Q(status=True, inscripcion__carrera__coordinacion__id=7),
                                    ...
                                    estadonotificacion=2).exclude(id__in=idgraduadosyanotif).order_by('-id')
if IS_DEBUG:
    graduados = Graduado.objects.filter(Q(status=True, inscripcion__carrera__coordinacion__id=7),
                                        fecharefrendacion__isnull=False, fecharefrendacion__year__gte=2022,
