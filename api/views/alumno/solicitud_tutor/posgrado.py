...
from api.forms.solicitud_tutor import SolicitudTutorMateriaForm, SolicitudTutorMatriculaForm
...
from api.serializers.alumno.solicitud_tutor import SolicitudTutorSoporteMateriaSerializer, \
    SolicitudTutorSoporteMatriculaSerializer


class PosgradoAPIView(APIView):
        ...
        try:
            ...
            if action == 'loadMaterias':
                ...
                try:
                    ...
                    ePeriodo = eMatricula.nivel.periodo
                    eProfesorMaterias = eMateria.profesormateria_set.filter(status=True, activo=True, tipoprofesor_id__in=[8])
            ...
            elif action == 'saveSolicitudTutorSoporteMatricula':
                with transaction.atomic():
                    try:
                        isNew = False
                        f = SolicitudTutorMatriculaForm(eRequest, eFiles)
                        f.initQuerySet(eRequest)
                        if not f.is_valid():
                            errors = []
                            for k, v in f.errors.items():
                        ...
                        id = eRequest.get('id', 0)
                        try:
                            eSolicitudTutorSoporteMatricula = SolicitudTutorSoporteMatricula.objects.get(matricula=eMatricula, pk=id)
                        except ObjectDoesNotExist:
                            eSolicitudTutorSoporteMatricula = SolicitudTutorSoporteMatricula(matricula=eMatricula)
                            isNew = True
                        eSolicitudTutorSoporteMatricula.tipo = f.cleaned_data['tipo']
                        eSolicitudTutorSoporteMatricula.descripcion = f.cleaned_data['descripcion']
                        archivo = f.files.get('archivo', None)
                        if archivo:
                            archivo.name = generar_nombre("solicitud_", archivo.name)
                            eSolicitudTutorSoporteMatricula.archivo = archivo
                        eSolicitudTutorSoporteMatricula.save(request)
                        if isNew is True:
                            log(u'Adicionó solicitud a tutor: %s' % eSolicitudTutorSoporteMatricula, request, "add")
                        else:
                            log(u'Edito solicitud a tutor: %s' % eSolicitudTutorSoporteMatricula, request, "add")
            ...
            elif action == 'deleteSolicitudTutorSoporteMatricula':
                with transaction.atomic():
                    try:
                        id = eRequest.get('id', 0)
                        try:
                            ...
                            raise NameError(u"Datos no encontrados")
                        deleteSolicitudTutorSoporteMatricula = eSolicitudTutorSoporteMatricula
                        eSolicitudTutorSoporteMatricula.delete()
                        log(u'Eliminó solicitud de tutor: %s' % deleteSolicitudTutorSoporteMatricula, request, "del")
                        return Helper_Response(isSuccess=True, data={}, message=f'Se elimino correctamente solicitud', status=status.HTTP_200_OK)
                    except Exception as ex:
                        transaction.set_rollback(True)
                        return Helper_Response(isSuccess=False, data={}, message=f'Error al guardar los datos: {ex.__str__()}', status=status.HTTP_200_OK)
    ...
    def get(self, request):
        ...
        try:
            ...
            filtro_1 = Q(materiaasignada__matricula=eMatricula) & Q(status=True)
            if search:
                ...
                filtro_1 = filtro_1 & filtro_s
            eSolicitudesTutor = SolicitudTutorSoporteMateria.objects.filter(filtro_1).order_by('-id')
            eSolicitudesTutor = SolicitudTutorSoporteMateriaSerializer(eSolicitudesTutor, many=True).data if eSolicitudesTutor.values("id").exists() else []
            filtro_2 = Q(matricula=eMatricula) & Q(status=True)
            eSolicitudesDocente = SolicitudTutorSoporteMatricula.objects.filter(filtro_2).order_by('-id')
            eSolicitudesDocente = SolicitudTutorSoporteMatriculaSerializer(eSolicitudesDocente, many=True).data if eSolicitudesDocente.values("id").exists() else []
            aData = {'eSolicitudes': {'eDocentes': eSolicitudesDocente, 'eTutores': eSolicitudesTutor}}
            return Helper_Response(isSuccess=True, data=aData, status=status.HTTP_200_OK)
        except Exception as ex:
            return Helper_Response(isSuccess=False, data={'eSolicitudes': {'eDocentes': [], 'eTutores': []}}, message=f'{ex.__str__()}', status=status.HTTP_200_OK)
